...
卡片上的 title / error / 条数就是结论；要细节再读 path。结果在 \`data/tool-results/{session}/{callId}.*\`。超阈值无全文，用 \`read_file(path, offset, maxChars)\` 分段取。
长文工具可带 \`expect_keywords\`（3–8）。禁止未读 path 假装已知全文。
**分段读**：\`read_file\`/\`read_article\` 返回 \`nextOffset\` 则翻页直到 \`truncated=false\`；看到 [TRUNCATED] 禁止下结论。`;
