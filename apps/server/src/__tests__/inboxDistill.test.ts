...
describe("PRD Inbox 蒸馏 状态×事件表", () => {
  ...
  it("R2 fetched 蒸馏成功：status+正文含 URL", async () => {
    ...
    const result = await services.inbox.distill({ ids: [item.id], garden: "knowledge", published: false });
  ...
  it("R4 已蒸馏再调幂等，不新建", async () => {
    ...
    inboxIds.push(item.id);
    const first = await services.inbox.distill({ ids: [item.id], garden: "knowledge", published: false });
    expect(first.distilled).toHaveLength(1);
    postIds.push(first.distilled[0]!.postId);
    const second = await services.inbox.distill({ ids: [item.id], garden: "knowledge", published: false });
  ...
  it("R5 幽灵 id 省略", async () => {
    ...
    const result = await services.inbox.distill({ ids: [ghost], garden: "knowledge", published: false });
    expect(result.distilled).toEqual([]);
