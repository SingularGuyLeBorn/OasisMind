...
name: paper-spine
description: >-
  Write, rewrite, or build a paper or report (journal, conference, report,
  review, competition) end to end, then output LaTeX/PDF/Word. OasisMind:
  this is a manuscript orchestrator, NOT a figure generator — for architecture
  diagrams and GenerateImage arrow wiring use academic-diagrams. Use only when
  the user names PaperSpine or explicitly asks to write a paper/LaTeX manuscript.
disable-model-invocation: true
---

# PaperSpine Orchestrator

**OasisMind：** 本 skill 管论文写作流程。架构图接线、`GenerateImage`、`fig-*.png` 走 `.cursor/skills/academic-diagrams/SKILL.md`。PaperSpine 铁律仍是 Never fabricate figures。
