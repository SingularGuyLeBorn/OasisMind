...
- 接到今天：把正确事实写进主叙述，读者只读一版。**禁止**再写 `> **2026-08 修订（不删上文）`。碰一篇就折一篇里的修订块。不要 HTML 注释存档旧错段。git 即历史快照。第 10/11/12 章除外（用户已删）。
- 不是商品、不是开源立项。不要售卖包装、不要 PUBLIC-SLICE。
- PowerShell 无 `&&`，用 `;`。
- 清单勾完不是停：按 brief 0.8 继续补知识点。禁止标题下只有一句话。
- **篇幅（用户 2026-08-30 拍板）：** 每篇专文正文 **汉字 ≥ 4000**（去掉 YAML frontmatter 后，用 `[\u4e00-\u9fff]` 计数）。不够就补机制、公式、插槽、失效、一手数字。**禁止**用水词、比喻段、「星辰大海 / 群星闪耀 / 一把钥匙」凑字。唯一例外：材料本身写不满（公开材料只有安全报告、没有架构表）——必须在文首写清「材料不够 4000」+ `[OM-FREEPLAY]`，仍然禁止注水。节首页是**地图**，不要写成第二份灌水文；地图可以短，专文不能短。
...
一篇算完成，当且仅当能回答：解决什么瓶颈、公式或算法步骤、和前代差在哪、何时失效、至少一条一手来源，**且汉字 ≥ 4000**（材料不够的例外必须打标）。文件存在 ≠ 写完。交卷前自计汉字，不够不要交。
