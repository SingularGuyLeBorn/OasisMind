...
name: humanwrite
description: >-
  Reproduce the author's Chinese voice (scy.md + good-writing). Use when
  de-AI-ifying OasisMind notes, shortening titles, removing agent meta from
  reader-facing prose, or the user says humanwrite / 人味 / 去AI味 / 标题太长.
---

# humanwrite

上游是账号计划的 `de-ai-writer-booster/good-writing`。本仓库再叠一层：读者正文必须像人写给读者，不能像 brief 写给模型。

**OasisMind 读者正文禁写（写进去 = 没人味）：**

- `地图（只读、不改）`、`本篇只链`、`积木只链不重推`、`B 档 SKU`、`材料类型（2026-08）`、`禁止假装有全文翻译`
- 标题里对 Agent 下命令：`不要读未发的旗舰规格`、`不要编发布日`
- 监工备忘：`第 14 章不给它开夹`、`点分号最多三层`
- 图注套话：`2026-08 自绘`、`浅色自绘`、`浅色图已有，不重画`、`（Fedus et al., 2021；2026-08 自绘）`、`旧深色稿仍在同夹，不再引用`、`配置数字以 … 为准`。论文出处写正文或文末参考文献，不要钉在图注末尾。不要把 `GenerateImage` prompt 留成 HTML 注释。不要写「文件保留，正文不再引用」。
- 磁盘备忘：`同夹 fig-xxx 本篇不引用`、`旧文件仍保留在同目录（不删）`、`不引用、不删除`、`提醒不要在本页展开`。夹里还有什么图、删不删，读者页不写。

这些话可以留在 `notes/`、Skill、supervisor。读者页只写判断和事实。

**标题：** H1 汉字尽量 ≤ 20（专名英文另计）。YAML `title` 可以稍长，但不要写成一段禁令。

**风格：** 先读 `references/scy.md`（作者指纹），再读下面 good-writing 原文。冲突时：知识库技术笔记跟 scy（短段、吐槽、数字），但**不要**把账号计划的 `post.md` 十节模板、禁止 Markdown 列表套到 llm-guide 上。公式、表、图解析保留。

**去 AI 味：** 同会话再 Read `.cursor/skills/humanizer-zh/SKILL.md`。数字、公式、来源 URL 不准改。
