...
### 2026-08-14 追加（memory-write-quality-and-cost）

- **feat(memory): 写入侧语义判定（Mem0 四元判定）**：新增 `infra/memoryWriteGate.ts`，在 `memoryRepository.write` 中、contentHash 命中与 debounce 之后、真正 create 之前，用轻量 LLM 对新增事实与 `MEMORY_USER_CREATABLE_TYPES` 邻居做 ADD/UPDATE/NOOP/CONFLICT 判定；`NOOP` 刷新目标强度不新建行，`UPDATE` 走 `supersedeUpdate` 进版本链，`CONFLICT` 双向挂链并存。`experience/persona` 跳过判定，避免污染经验/人格记忆。配置 `config.yaml memory.writeDedup` 默认启用，超时/异常/空邻居回退旧行为（ADD）。新增 `__tests__/memoryWriteGate.test.ts`（7 例，含异常回退、experience 跳过、空邻居零 LLM 调用等负向）。设计见 `design-decisions.md`「Memory 写入侧语义判定」。
- **feat(llm): 规划轮/执行轮角色化拆价（默认关闭）**：新增 `infra/loop/roundModel.ts` 纯函数 `resolveRoundModel`，`LlmTransport.complete` 扩展 `modelOverride` 参数；`transports.ts` 每次调用以 `modelOverride ?? baseModel` 生效并回记 `turn.model`；`reactLoop.ts` 主循环与收尾合成轮按当前轮次传入覆盖。`config.yaml llm.roleSplit` 默认 `enabled: false`，`planningRounds` 默认 1。Turn Snapshot 冻结的 base model 不动，仅 per-call 覆盖，保证既有测试零回归。新增 `__tests__/roleSplitModel.test.ts`（5 例）。设计见 `design-decisions.md`「LLM 角色化拆价」。
- **fix(evolution): IVE direction 归因规则层修复**：扩展 `agentEvolution.ts` `attributeFailure(toolCalls, { producedOutput? })`；规则顺序：①工具错误签名 → implementation；②无工具错误但 `producedOutput === false`（content 为空/仅空白）→ direction，failureReason 为「工具调用无报错但未产出有效内容，疑似任务方向/理解偏差」；③否则 unknown。`accumulateExperience` 失败分支透传 `producedOutput: !!result.content.trim()`，使 experience JSON 的 `failureKind` 真实反映 direction。更新 `agentEvolutionOptimize.test.ts` 覆盖 direction 路径与防御路径。设计见 `design-decisions.md`「IVE direction 归因规则层修复」。
