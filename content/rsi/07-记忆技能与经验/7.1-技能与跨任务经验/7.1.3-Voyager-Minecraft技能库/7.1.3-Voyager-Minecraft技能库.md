---
title: Voyager：冻 GPT-4，只长可执行技能库
date: 2026-08-31
as_of: 2026-08-31
category: 记忆技能与经验
published: true
...
# Voyager：冻 GPT-4，只长可执行技能库

Minecraft 里一只 Agent 自己造木镐、挖铁、偶尔摸到钻石，技能以 JavaScript 函数留下，下次还能调。新闻标题常写成「终身学习、无需人类」。论文原句更窄：GPT-4 **黑盒查询**，**不微调参数**。会涨的是技能库 $H_t$，不是改进器。把「技能越写越复杂」听成花园式 (2)，[Argus](../../../09-自动研究与科学发现/9.1-自改代码与元智能体/9.1.1-Argus-Verification-Gated/9.1.1-Argus-Verification-Gated.md) 文里点名的邻居就被升格成 RSI。

本篇是 Harness 层里「可执行技能库」的前史样板。[SkillEvolver](../7.1.1-SkillEvolver-元技能/7.1.1-SkillEvolver-元技能.md) 冻 CLI、写领域 `SKILL.md`；[ACE](../7.1.2-ACE-Agentic-Context-Engineering/7.1.2-ACE-Agentic-Context-Engineering.md) 冻 $\theta$、写 playbook。Voyager 更早：冻 `gpt-4-0314`，把过了自验证的代码提交进 Chroma。门比 Argus 弱——自验证还是 GPT-4 看背包，蜘蛛丝可以骗过「打赢了蜘蛛」。**不是** RSI：课程提示、Mineflayer API、嵌入模型、GPT-4 本身都不进 $S'$。**不是** DreamerV3 / VPT 那种改权重的 Minecraft 智能体。一手：Wang, Xie, Jiang, Mandlekar, Xiao, Zhu, Fan, Anandkumar，[arXiv:2305.16291](https://arxiv.org/abs/2305.16291)（2023-05-25）；代码 [MineDojo/Voyager](https://github.com/MineDojo/Voyager)。数字以 HTML Table 1–2 与 §3.4 消融为准。
...
和邻居先划线。[Reflexion](../7.1.4-Reflexion-言语反思记忆/7.1.4-Reflexion-言语反思记忆.md) 把自然语言反思留下，不提交可执行函数。AutoGPT 拆子目标，论文重实现里没有技能库、没有自验证、没有开放课程。Argus 把技能、路由、合约全部过门，门在独立审计。Voyager 的门是 GPT-4 看当前背包是否像「任务完成」。DreamerV3 学世界模型去收钻石，那是 Model 层，本篇不混。
...
控制原语是人写的 Mineflayer 包装，不是技能库搜出来的。Agent 能调 `exploreUntil`、`craftItem`，不能改这些函数的源码。幻觉会调用提示里没有的 API，执行报错后再改。身体在墙外，和 [Absolute Zero](../../../02-模型自改进与自训练/2.2-自适应与递归解题/2.2.3-Absolute-Zero-Reasoner/2.2.3-Absolute-Zero-Reasoner.md) 的 Python 解释器是同一类：环境能验对错，但不让 Agent 改验的规则。Minecraft 合成表、矿物层级、圆石不能当燃料，全是游戏引擎的 $U$，不是 $H_t$。

GPT-3.5 在这套系统里干三件便宜的事：给技能写自然语言描述、自问自答给课程当上下文、检索前写解题建议。代码生成换成 3.5 会崩，描述和检索查询仍用 3.5。分层是成本，不是把改进器拆成两只互相改的模型。[R-Zero](../../../02-模型自改进与自训练/2.2-自适应与递归解题/2.2.4-R-Zero-挑战者解题器/2.2.4-R-Zero-挑战者解题器.md) 的挑战者 / 解题器是两只可训练克隆；Voyager 的 4 与 3.5 都冻着，只是账单不同。
...
和 [9.1.2 Auto-Research](../../../09-自动研究与科学发现/9.1-自改代码与元智能体/9.1.2-Karpathy-Auto-Research/9.1.2-Karpathy-Auto-Research.md) 对照：那边改 `train.py`，考官是墙外的 val_bpb；这边改的是游戏技能，考官是 GPT-4 自检加 Mineflayer 能否跑通。都冻基座。Auto-Research 的交卷是训练脚本，Voyager 的交卷是 `craftStoneShovel`。[1.5.1 CS329A](../../../01-研究框架与术语/1.5-资源与阅读路径/1.5.1-CS329A-Skill入口/1.5.1-CS329A-Skill入口.md) 的 skill 是给人读的入口，不是 Minecraft 函数。不要把三篇的「skill」收成一个词。
...
$H_t$ 变了，下次检索能调到 `craftStoneShovel`。单轮成立。改进器——出题提示、验证提示、GPT-4、嵌入模型——下一轮还是同一份。$I'\subseteq S'$ 不成立。混元台阶上这是 L2 附近的脚手架切片：留下的是技能，不是提议/选择程序。L3 要改改进器；Voyager 改的是技能档案。不要用 63 种物品给 [DGM](../../../09-自动研究与科学发现/9.1-自改代码与元智能体/9.1.3-DGM-达尔文哥德尔机/9.1.3-DGM-达尔文哥德尔机.md) 的「改自己的 Python」背书。DGM 的 $I$ 有一部分已经在仓库里；Voyager 的 $I$ 在 OpenAI API 后面。

和同层钉死。[SkillEvolver](../7.1.1-SkillEvolver-元技能/7.1.1-SkillEvolver-元技能.md) 的元技能冻着，领域 `SKILL.md` 可携带，门是部署失败。Voyager 的技能是游戏内 JavaScript，门是 GPT-4 自检，元提示不自改。[ACE](../7.1.2-ACE-Agentic-Context-Engineering/7.1.2-ACE-Agentic-Context-Engineering.md) 的 playbook 是条目，合并非 LLM；Voyager 的值是可执行函数，解释器是 Minecraft。[Argus](../../../09-自动研究与科学发现/9.1-自改代码与元智能体/9.1.1-Argus-Verification-Gated/9.1.1-Argus-Verification-Gated.md) 拒绝「单靠变长上下文自评进度」；Voyager 正好走那条：自验证看文本盘面。Reflexion 是邻居里更浅的：反思自然语言，不提交函数。AutoGPT 借库能涨，说明库是资产，编排仍是人写的。

[SEAGym](../../../10-评测与安全边界/10.2-SEAGym-Harness评测环境/10.2-SEAGym-Harness评测环境.md) 冻 $M$ 测 $H_t$ 的 OOD。Voyager 的 Table 2 是换世界、空背包的零样本，近似「新任务」，不是 Harbor 协议，数字不能和 AHE +17.1 横加。[RSIBench](../../../10-评测与安全边界/10.1-RSIBench-Data/10.1-RSIBench-Data.md) 冻训练栈只改数据；Voyager 连 $\theta$ 都不动。[System Card](../../../10-评测与安全边界/10.4-System-Card-RSI/10.4-System-Card-RSI.md) 的 RSI Index 是预备度代理任务；63 种物品不是 High。
...
迭代提示看起来像「自己改自己」，改的是这一轮的 JavaScript 草稿，不是改 GPT-4，也不是改课程模板。草稿过门才进 $H_t$。没过门的草稿丢掉，和 DGM 把失败子代留在档案里不同。开放档案能追溯作弊；Voyager 的库默认只留成功函数，失败轨迹主要留在 ckpt 的事件日志里，主文没有拿日志做可靠性审计。[10.3 可靠性](../../../10-评测与安全边界/10.3-可靠性与独立监督/10.3-可靠性与独立监督.md) 所要求的「证据在更新边界外」，这里的边界外证据是 Mineflayer 跑没跑通，以及换世界之后 Table 2 还能否做对。自验证本身在边界内。
...
同层：[9.1.1 Argus](../../../09-自动研究与科学发现/9.1-自改代码与元智能体/9.1.1-Argus-Verification-Gated/9.1.1-Argus-Verification-Gated.md)、[7.1.1 SkillEvolver](../7.1.1-SkillEvolver-元技能/7.1.1-SkillEvolver-元技能.md)、[7.1.2 ACE](../7.1.2-ACE-Agentic-Context-Engineering/7.1.2-ACE-Agentic-Context-Engineering.md)、[7.1.5 ExpeL](../7.1.5-ExpeL-跨题经验洞察/7.1.5-ExpeL-跨题经验洞察.md)、[7.1.6 Dynamic Cheatsheet](../7.1.6-Dynamic-Cheatsheet-测试时备忘录/7.1.6-Dynamic-Cheatsheet-测试时备忘录.md)、[7.1.8 AWM](../7.1.8-AWM-工作流记忆/7.1.8-AWM-工作流记忆.md)、[6.2.1 LATM](../../../06-工具学习与程序编排/6.2-工具创建与文档优化/6.2.1-LATM-函数缓存造工具/6.2.1-LATM-函数缓存造工具.md)、[5.2.3 ReAct](../../../05-搜索规划与提示优化/5.2-推理搜索与规划/5.2.3-ReAct-推理与动作/5.2.3-ReAct-推理与动作.md)。判定：[1.1 术语](../../../01-研究框架与术语/1.1-RSI-术语辨析/1.1-RSI-术语辨析.md)。读完应能把「Minecraft 里越玩越强」翻译成：冻结的 GPT-4，加上一份会涨的 JavaScript 库，身体和出题规则仍在墙外。越玩越强的是 $H_t$，不是 $I$。课程提示一行不改，钻石仍可能三局里只有一局摸到。这不是失败叙事，是 Table 1 自己写在格子里的分数。请去原文核那一格。
...
3. 本花园：[Argus](../../../09-自动研究与科学发现/9.1-自改代码与元智能体/9.1.1-Argus-Verification-Gated/9.1.1-Argus-Verification-Gated.md)；[SkillEvolver](../7.1.1-SkillEvolver-元技能/7.1.1-SkillEvolver-元技能.md)；[ACE](../7.1.2-ACE-Agentic-Context-Engineering/7.1.2-ACE-Agentic-Context-Engineering.md)。
