---
title: ReTool：代码解释器进 PPO rollout，奖励仍只看最终答案
date: 2026-08-31
as_of: 2026-08-31
category: 工具学习与程序编排
published: true
...
# ReTool：代码解释器进 PPO rollout，奖励仍只看最终答案
...
相关工作把 Tool-Integrated Reasoning 收成一条线：Chen 等 2025 的 R1 风格 SFT 仍绑在那份代码 CoT 分布上；并发的 [ToRL](../6.1.3-ToRL-从基座做工具RL/6.1.3-ToRL-从基座做工具RL.md)（Li 等，[arXiv:2503.23383](https://arxiv.org/abs/2503.23383)）在 Qwen2.5-Math 的 1.5B / 7B 上做工具 RL，7B AIME24 greedy 43.3；作者写成规模不够、分数不理想，那是站在 32B、32 次平均上看。ReTool 把骨干拉到 32B，工具钉死为代码解释器，不做多 API 选择。不要和 [ToolLLM](../6.1.5-ToolLLM-RapidAPI轨迹SFT/6.1.5-ToolLLM-RapidAPI轨迹SFT.md) / [Gorilla](../6.1.4-Gorilla-API调用微调/6.1.4-Gorilla-API调用微调.md) 的 API 轨迹 SFT 收成一张表（那边 TorchHub 0-shot 59.13，Oracle 67.20 不要改本篇 67.0；ToolLLM 均 pass 66.7 是 ToolEval，更不要横加），也不要和 Search-R1 的搜索引擎 RL 收成一篇。并发的 [ToolRL](../6.1.2-ToolRL-多工具奖励设计/6.1.2-ToolRL-多工具奖励设计.md) 才是多 API、拆槽位奖励，3B BFCL 52.98，不要和本篇 AIME 67.0 横加。PoT / PAL 是提示侧把计算交给 Python，权重不动；本篇的差是：同样接解释器，但 \(\theta\) 会被 +1/−1 推过。差在训练，不在「会不会 print」。
...
超参：AdamW，学习率 \(1\times 10^{-6}\)，最大序列 **16384**，mini-batch **512**，KL 系数 **0.0**。主骨干 Qwen2.5-32B-Instruct。实现写 VeRL。KL 关掉，等于不把策略钉在冷启动 SFT 附近。R1 一类常用 GRPO 加小 KL；这边明确写 PPO、KL=0。花园不把「没用 GRPO」听成方法失败，只记：更新规则是人钉的 \(I\)，和 [Tufa](../../../02-模型自改进与自训练/2.1-自生成数据与自奖励/2.1.3-Tufa-Labs-自奖励/2.1.3-Tufa-Labs-自奖励.md) 的 GRPO、Absolute Zero 的 TRR++ 不是同一配方。RL 题库项目页钉成 [DAPO-Math-17k](https://huggingface.co/datasets/BytedTsinghua-SIA/DAPO-Math-17k)（Yu 等，[arXiv:2503.14476](https://arxiv.org/abs/2503.14476)），验证用 AIME 2024 / 2025。论文 HTML 方法段没把 17k 印进 Table 1。花园把题库记成项目页事实，不当成主表格子。DAPO 自己的论文在讲解耦 clip、动态采样那一套；ReTool 借用的是题和规则验证器，不是把 DAPO 算法整份搬进式 (1)。
...
不要用 67.0 改 [Dynamic Cheatsheet](../../../07-记忆技能与经验/7.1-技能与跨任务经验/7.1.6-Dynamic-Cheatsheet-测试时备忘录/7.1.6-Dynamic-Cheatsheet-测试时备忘录.md) 的 AIME 50.0：那边是 Claude 3.5 Sonnet 在测试流上改备忘录，权重冻着，30 题、协议另一套。不要改 [ANN](../../../08-多智能体协作与工作流/8.2-多智能体拓扑与协作/8.2.6-ANN-层状文本反传/8.2.6-ANN-层状文本反传.md) Figure 4 的 MATH→AIME 迁移，那边没有 AIME 格子。不要改 MaAS 的 MATH 51.82。AIME 年份、采样次数、温度、是否接 CI，任何一项不同就禁止横加。
...
和邻居钉死。[Absolute Zero](../../../02-模型自改进与自训练/2.2-自适应与递归解题/2.2.3-Absolute-Zero-Reasoner/2.2.3-Absolute-Zero-Reasoner.md) 的解释器验 \((p,i,o)\)，题可以从恒等函数长出来；这边题是人出的 17k，解释器只服务解题 rollout。两边 Python 都不可被 \(\theta\) 改写。[R-Zero](../../../02-模型自改进与自训练/2.2-自适应与递归解题/2.2.4-R-Zero-挑战者解题器/2.2.4-R-Zero-挑战者解题器.md) 连执行器也不要，金标是多数票；这边金标是 AIME / DAPO 的规则等价。[LADDER](../../../02-模型自改进与自训练/2.2-自适应与递归解题/2.2.2-LADDER-递归拆题/2.2.2-LADDER-递归拆题.md) 从人给的积分根往下拆变体，验证器是数值积分，TTRL 答完还回滚；这边权重留下，课程不是变体树。[Tufa](../../../02-模型自改进与自训练/2.1-自生成数据与自奖励/2.1.3-Tufa-Labs-自奖励/2.1.3-Tufa-Labs-自奖励.md) 冻 LLM 裁判给 0/1；这边冻的是解释器加字符串匹配，裁判更硬，仍不是递归。[SEAL](../../../02-模型自改进与自训练/2.2-自适应与递归解题/2.2.1-SEAL-自适配语言模型/2.2.1-SEAL-自适配语言模型.md) 内环 LoRA 改 \(\theta\)，外环用下游 \(\tau\) 筛 self-edit，测试题可以回滚；这边 PPO 留下的权重跨题还在，筛的却是 DAPO 金标而不是模型自己写的编辑。[SPIN](../../../02-模型自改进与自训练/2.1-自生成数据与自奖励/2.1.1-SPIN-自对弈微调/2.1.1-SPIN-自对弈微调.md) 的赢家分布是人类 SFT，没有沙箱。[LATM](../../6.2-工具创建与文档优化/6.2.1-LATM-函数缓存造工具/6.2.1-LATM-函数缓存造工具.md) 留下一类题的 Python 函数，造工具提示冻着，不改 \(\theta\)。[ReAct](../../../05-搜索规划与提示优化/5.2-推理搜索与规划/5.2.3-ReAct-推理与动作/5.2.3-ReAct-推理与动作.md) 冻 PaLM，想、做、看随题清空。[CRITIC](../../../04-反思批评与反馈优化/4.2-CRITIC-工具交互批评/4.2-CRITIC-工具交互批评.md) 也接解释器，但是 L0 任务内批评。[Dynamic Cheatsheet](../../../07-记忆技能与经验/7.1-技能与跨任务经验/7.1.6-Dynamic-Cheatsheet-测试时备忘录/7.1.6-Dynamic-Cheatsheet-测试时备忘录.md) 的 50.0 是备忘录，不是 PPO。[ToolLLM](../6.1.5-ToolLLM-RapidAPI轨迹SFT/6.1.5-ToolLLM-RapidAPI轨迹SFT.md) / [Gorilla](../6.1.4-Gorilla-API调用微调/6.1.4-Gorilla-API调用微调.md) 是 API 轨迹 SFT，没有这张 AIME 表；那边均 pass 66.7 不要改本篇 67.0。
...
同层：[2.2.3 Absolute Zero](../../../02-模型自改进与自训练/2.2-自适应与递归解题/2.2.3-Absolute-Zero-Reasoner/2.2.3-Absolute-Zero-Reasoner.md)、[2.2.4 R-Zero](../../../02-模型自改进与自训练/2.2-自适应与递归解题/2.2.4-R-Zero-挑战者解题器/2.2.4-R-Zero-挑战者解题器.md)、[2.2.2 LADDER](../../../02-模型自改进与自训练/2.2-自适应与递归解题/2.2.2-LADDER-递归拆题/2.2.2-LADDER-递归拆题.md)、[2.2.1 SEAL](../../../02-模型自改进与自训练/2.2-自适应与递归解题/2.2.1-SEAL-自适配语言模型/2.2.1-SEAL-自适配语言模型.md)、[6.1.2 ToolRL](../6.1.2-ToolRL-多工具奖励设计/6.1.2-ToolRL-多工具奖励设计.md)、[6.1.3 ToRL](../6.1.3-ToRL-从基座做工具RL/6.1.3-ToRL-从基座做工具RL.md)、[6.1.4 Gorilla](../6.1.4-Gorilla-API调用微调/6.1.4-Gorilla-API调用微调.md)、[6.1.5 ToolLLM](../6.1.5-ToolLLM-RapidAPI轨迹SFT/6.1.5-ToolLLM-RapidAPI轨迹SFT.md)。信号：[1.3 模仿与 RLVR](../../../01-研究框架与术语/1.3-模仿学习与RLVR/1.3-模仿学习与RLVR.md)。Harness 侧工具：[6.2.1 LATM](../../6.2-工具创建与文档优化/6.2.1-LATM-函数缓存造工具/6.2.1-LATM-函数缓存造工具.md)、[5.2.3 ReAct](../../../05-搜索规划与提示优化/5.2-推理搜索与规划/5.2.3-ReAct-推理与动作/5.2.3-ReAct-推理与动作.md)、[4.2 CRITIC](../../../04-反思批评与反馈优化/4.2-CRITIC-工具交互批评/4.2-CRITIC-工具交互批评.md)。评测纪律：[10.3 可靠性](../../../10-评测与安全边界/10.3-可靠性与独立监督/10.3-可靠性与独立监督.md)。
...
6. 本花园：[2.2.3 Absolute Zero](../../../02-模型自改进与自训练/2.2-自适应与递归解题/2.2.3-Absolute-Zero-Reasoner/2.2.3-Absolute-Zero-Reasoner.md)；[1.3 模仿与 RLVR](../../../01-研究框架与术语/1.3-模仿学习与RLVR/1.3-模仿学习与RLVR.md)；[1.1 术语](../../../01-研究框架与术语/1.1-RSI-术语辨析/1.1-RSI-术语辨析.md)。
