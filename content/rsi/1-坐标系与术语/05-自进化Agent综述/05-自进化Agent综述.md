...
**工具。** 训练侧用轨迹 SFT（ToolLLM、Gorilla）或 RL（ReTool、ToolRL）。推理侧改工具文档写法（EASYTOOL）或用树搜索选工具。再往前是造工具：[LATM](../../3-Harness层-Agent运行时/42-LATM-函数缓存造工具/42-LATM-函数缓存造工具.md) 按类写出可复用 Python 并摊销，GPT-4 造、GPT-3.5 调，中国剩余 CoT 两边 0.0、LATM 100.0，造工具提示冻着；CREATOR 按题写文档和代码，ChatGPT 上 MATH 59.7%，不要和 LATM 的 79.7 横加。AgentOptimiser 把工具当可学习权重。造出来的可执行代码若只服务当前任务、不进入保留的 Agent 状态，偏 Artifact；若下次任务默认带上这套工具表，偏 Harness。LATM 的函数缓存跨实例还在，属于后一档，改进器仍在墙外。
...
自进化多智能体把工作流写成搜索，空间有三块：结构（谁连谁）、语义（角色和指令）、能力（用哪只骨干）。提示优化在固定拓扑上改角色说明（DSPy Assertions、AutoAgents）。拓扑优化分两家：代码级工作流（[AutoFlow](../../3-Harness层-Agent运行时/47-AutoFlow-自然语言工作流RL/47-AutoFlow-自然语言工作流RL.md) 的自然语言程序 + RL，OpenAGI 上相对人工 CoRE 约 45% 相对涨幅不是准确率柱；[AFlow](../../3-Harness层-Agent运行时/43-AFlow-工作流MCTS/43-AFlow-工作流MCTS.md) 的类型化算子图 + MCTS，六集均分 80.3，摘要 5.7% 不是每列都涨 5.7 个百分点，19.5% 是相对本表 ADAS 67.2 的相对涨幅，Claude 优化器冻着；[ScoreFlow](../../3-Harness层-Agent运行时/45-ScoreFlow-Score-DPO工作流/45-ScoreFlow-Score-DPO工作流.md) 把代码表示抬到生成器 LoRA 上做 Score-DPO，六集均分 85.3，摘要 8.2% 是对八条基线均分约 77.0 的百分点差，不是相对 AFlow 重跑 80.6；[MAS-GPT](../../3-Harness层-Agent运行时/48-MAS-GPT-一次前向吐MAS/48-MAS-GPT-一次前向吐MAS.md) 一次前向吐出可执行 MAS 代码，Llama-3-70B 八列均分 65.47，3.89% 是对自洽的百分点差），以及通信图（[GPTSwarm](../../3-Harness层-Agent运行时/44-GPTSwarm-通信图边概率/44-GPTSwarm-通信图边概率.md) 学边概率，GAIA 90.2% 是相对 9.70 的相对涨幅且主表未跑边优化；[G-Designer](../../3-Harness层-Agent运行时/49-G-Designer-任务自适应通信图/49-G-Designer-任务自适应通信图.md) 生成任务自适应图，MMLU 84.50、HumanEval 89.90，0.3 是百分点；AgentPrune 剪边省 token）。统一优化承认提示和拓扑互相卡：ADAS 把提示、工作流、工具写成 Python，元 Agent 迭代生成再评价——花园 [ADAS 专文](../../3-Harness层-Agent运行时/07-ADAS-Meta-Agent-Search/07-ADAS-Meta-Agent-Search.md) 钉死的是「元 Agent 自己不被搜」。AFlow 的 ADAS 对照行 MBPP 53.4 不要改 ADAS 专文的 MGSM 53.4。[MASS](../../3-Harness层-Agent运行时/46-MASS-提示拓扑分阶段/46-MASS-提示拓扑分阶段.md) 分三阶段近似联合优化：先局部调各 Agent 提示，再在剪过的空间搜拓扑，再全局调提示；Gemini 1.5 Pro 八列均分 78.79，不要和 AFlow 专文 80.3 横加。学习派把超网或层状 Agent 队当成可采样对象（MaAS、ANN）。
...
| 代码级工作流当空间 | Harness（元 Agent 常冻） | ADAS；[AFlow](../../3-Harness层-Agent运行时/43-AFlow-工作流MCTS/43-AFlow-工作流MCTS.md)；[ScoreFlow](../../3-Harness层-Agent运行时/45-ScoreFlow-Score-DPO工作流/45-ScoreFlow-Score-DPO工作流.md)；Gödel 才把运行时打开 |
| 通信图边概率 | Harness（REINFORCE / VGAE 冻） | [GPTSwarm](../../3-Harness层-Agent运行时/44-GPTSwarm-通信图边概率/44-GPTSwarm-通信图边概率.md)；[G-Designer](../../3-Harness层-Agent运行时/49-G-Designer-任务自适应通信图/49-G-Designer-任务自适应通信图.md) MMLU 84.50；AgentPrune 仍裸 |
