...
**记忆。** 综述把训练时改权重的知识编辑划出去，只谈推理时调度：短时压缩、摘要、选择性保留（ReadAgent、MemoryBank 按遗忘曲线更新）；长时 RAG、图索引、SQL（[MemGPT](../../3-Harness层-Agent运行时/36-MemGPT-操作系统式记忆/36-MemGPT-操作系统式记忆.md)、[HippoRAG](../../3-Harness层-Agent运行时/38-HippoRAG-海马索引检索/38-HippoRAG-海马索引检索.md)、ChatDB）。MemGPT 专文钉死：窗口当 RAM，DMR 上 GPT-4 Turbo 35.3→93.4，裁判慷慨，换页政策冻着，不是术语式 (2)。[A-Mem](../../3-Harness层-Agent运行时/37-A-Mem-卡片盒记忆/37-A-Mem-卡片盒记忆.md) 把原子笔记连边并改旧卡片：LoCoMo 上 GPT-4o-mini 多跳 27.02 对对照 MemGPT 26.65，不是两倍；对抗题全上下文 69.23 更高；三份提示冻着。[HippoRAG](../../3-Harness层-Agent运行时/38-HippoRAG-海马索引检索/38-HippoRAG-海马索引检索.md) 把语料收成开放知识图再 PPR：2Wiki R@5 68.2→89.1 是百分点，HotpotQA 单步召回低于 ColBERT，OpenIE 提示冻着。Reflexion 把任务反馈写成可存的句子再读回来——留下的是文本记忆，不是 \(\theta\)。[ExpeL](../../3-Harness层-Agent运行时/32-ExpeL-跨题经验洞察/32-ExpeL-跨题经验洞察.md) 再把训练轨迹抽成跨题洞察，评测可以一次交卷。[Dynamic Cheatsheet](../../3-Harness层-Agent运行时/33-Dynamic-Cheatsheet-测试时备忘录/33-Dynamic-Cheatsheet-测试时备忘录.md) 没有训练 / 评测切分，测试流上题题改 \(M\)，策展没有金标。[AWM](../../3-Harness层-Agent运行时/35-AWM-工作流记忆/35-AWM-工作流记忆.md) 把网页轨迹收成可再执行的工作流，WebArena 23.5→35.5，摘要 51.1% / 24.6% 是相对涨幅，不要听成准确率柱；在线入库走语言模型裁判，诱导提示冻着。控制「存什么、何时取、丢什么」的策略若被优化器改写，属于 Harness。
...
| 提示 / 记忆 / 工具表 / 拓扑 | Harness | Argus / ACE / SkillEvolver / ExpeL / Dynamic Cheatsheet / BoT / AWM / MemGPT / A-Mem / HippoRAG / ADAS / STOP / DGM / Auto-Research |
