...
**工具。** 训练侧用轨迹 SFT（ToolLLM、Gorilla）或 RL（[ReTool](../../2-Model层-训练时自改进/08-ReTool-代码解释器RL/08-ReTool-代码解释器RL.md) 的 PPO 交错代码解释器，Qwen2.5-32B AIME2024 **67.0** / 400 步，换 R1-Distill 才到 **72.5**，不要和 67.0 收成一只；[ToolRL](../../2-Model层-训练时自改进/09-ToolRL-多工具奖励设计/09-ToolRL-多工具奖励设计.md) 的 GRPO 拆格式与槽位，3B BFCL **52.98**，摘要 17% / 15% 是跨榜汇总不是某一格）。推理侧改工具文档写法（EASYTOOL）或用树搜索选工具。再往前是造工具：[LATM](../../3-Harness层-Agent运行时/42-LATM-函数缓存造工具/42-LATM-函数缓存造工具.md) 按类写出可复用 Python 并摊销，GPT-4 造、GPT-3.5 调，中国剩余 CoT 两边 0.0、LATM 100.0，造工具提示冻着；CREATOR 按题写文档和代码，ChatGPT 上 MATH 59.7%，不要和 LATM 的 79.7 横加。AgentOptimiser 把工具当可学习权重。造出来的可执行代码若只服务当前任务、不进入保留的 Agent 状态，偏 Artifact；若下次任务默认带上这套工具表，偏 Harness。LATM 的函数缓存跨实例还在，属于后一档，改进器仍在墙外。
...
| 综述格子 | 花园落点 | 样板 |
|----------|----------|------|
| 训 LLM 行为 | Model | SPIN / Self-Rewarding / Tufa / LADDER / SEAL / Absolute Zero / R-Zero / [ReTool](../../2-Model层-训练时自改进/08-ReTool-代码解释器RL/08-ReTool-代码解释器RL.md) / [ToolRL](../../2-Model层-训练时自改进/09-ToolRL-多工具奖励设计/09-ToolRL-多工具奖励设计.md) |
