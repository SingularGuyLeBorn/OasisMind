...
下文是 2025–08 对专栏的摘要，**不当成本库术语定义**。原文把 Artifacts / Harness / Model「三者都算 RSI」，与本花园 [01 术语](../01-RSI-术语辨析/01-RSI-术语辨析.md) 冲突：Artifact 迭代默认不是 RSI；Harness 自改是弱自改进；Model 单轮训练也不是递归。数字与机构主张以各专文一手为准，不要用本节口算。本体：[02 Model–Harness–Artifact](../02-Model-Harness-Artifact/02-Model-Harness-Artifact.md)。
