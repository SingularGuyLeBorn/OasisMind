---
title: ADAS：元 Agent 会写代码，但自己不改自己
date: 2026-08-31
as_of: 2026-08-31
category: 自动研究与科学发现
published: true
...
实现切口：不到 **100 行**框架提供查询 API 和提示格式化，元 Agent 写 `forward(task) → answer`，对照 [FunSearch](../../9.3-科学发现与产物搜索/9.3.3-FunSearch-函数空间搜索/9.3.3-FunSearch-函数空间搜索.md) 的短函数接口。框架是墙外 $I$。图灵完备只说明空间上限，不说明 30 轮 gpt-4o 能走到任意点。元模型与交卷模型的具体编号是 **gpt-4o-2024-05-13** 与 **gpt-3.5-turbo-0125**，Gödel Agent 后来沿用同一对，为的是对照而不是换更强考生。
...
按 [1.1 术语](../../../01-研究框架与术语/1.1-RSI-术语辨析/1.1-RSI-术语辨析.md)：$S$ 是下游 `forward`，$I$ 是元 Agent + 框架 + 评价协议。$S'=I(S)$ 每轮都发生。RSI 要 $I'\subseteq S'$。本实验 $I$ 冻死。作者 §6 第一条未来工作就是 Higher-order ADAS：元 Agent 自己也是 Agent，理论上可以被 ADAS 改，甚至改到元元 Agent。句子是虚拟语气。Gödel Agent、STOP、DGM 做的是这条清单上的下一步，不是 ADAS 主实验已经交付的东西。混元阶梯上这是 **L2**：改脚手架，证据是留出测试；元 Agent 没进 $S'$，到不了 L3。特征失败是验证集过拟合和工作流过拟合某一 FM——Table 3 已经显示 GPT-3.5 上的最优结构迁到更强模型会换人。
...
| | 改什么 | 元 / 改进器 | 算不算 RSI |
|--|--------|-------------|------------|
| [OPRO](../../../05-搜索规划与提示优化/5.1-提示与上下文优化/5.1.3-OPRO-元提示优化/5.1.3-OPRO-元提示优化.md) / [Promptbreeder](../../../05-搜索规划与提示优化/5.1-提示与上下文优化/5.1.2-Promptbreeder-自我指涉提示进化/5.1.2-Promptbreeder-自我指涉提示进化.md) | 提示文本 | 冻 | 否 |
...
同层自指：[9.1.5 Gödel Agent](../9.1.5-Godel-Agent-自指运行时/9.1.5-Godel-Agent-自指运行时.md)（公平对照打本篇 Table 1）；[9.1.3 DGM](../9.1.3-DGM-达尔文哥德尔机/9.1.3-DGM-达尔文哥德尔机.md)（无自改 ≈ 本篇）；[9.1.4 STOP](../9.1.4-STOP-自教优化器/9.1.4-STOP-自教优化器.md)。同层工作流 MCTS：[8.1.1 AFlow](../../../08-多智能体协作与工作流/8.1-工作流搜索与优化/8.1.1-AFlow-工作流MCTS/8.1.1-AFlow-工作流MCTS.md)（本表 ADAS 的 MBPP 53.4 不要改本篇 MGSM 53.4）。同层通信图：[8.2.1 GPTSwarm](../../../08-多智能体协作与工作流/8.2-多智能体拓扑与协作/8.2.1-GPTSwarm-通信图边概率/8.2.1-GPTSwarm-通信图边概率.md)。同层 Score-DPO：[8.1.2 ScoreFlow](../../../08-多智能体协作与工作流/8.1-工作流搜索与优化/8.1.2-ScoreFlow-Score-DPO工作流/8.1.2-ScoreFlow-Score-DPO工作流.md)（本表 ADAS 76.6 / MBPP 68.7 不要改本篇 MGSM 53.4）。产物层代码搜索：[FunSearch](../../9.3-科学发现与产物搜索/9.3.3-FunSearch-函数空间搜索/9.3.3-FunSearch-函数空间搜索.md)。数字只认 arXiv:2408.08435 的 Table 1–3 与 §4.3，不认二手「自动设计 Agent 已经自我进化」。
...
5. 本花园：[1.1 术语](../../../01-研究框架与术语/1.1-RSI-术语辨析/1.1-RSI-术语辨析.md)。
