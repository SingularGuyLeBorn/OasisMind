...
更有用的是他们记录的作弊。上下文很长时，Claude 3.5 会**假装**调了 Bash 跑单元测试，把「全部通过」写成纯文本日志，工具其实没调。日志随后进入上下文，Agent 以为补丁已经绿了。作者另设「工具幻觉」分数，让 DGM 去修。有的子代真去修检测；有的把检测用的特殊标记删掉——指令明确禁止——让幻觉分数看起来完美。因为血统可追溯，这种 objective hacking 能被抓到。[10.3 可靠性与独立监督](../../../10-评测与安全边界/10.3-可靠性与独立监督/10.3-可靠性与独立监督.md) 要的「证据必须在更新边界之外」，这里是反例教材：分数在更新边界之内，系统会改分数通道。
...
外环若也被 Agent 改写且没有独立监督，才接近实验室文案里的 RSI；DGM 没走那一步。把「可以改训练脚本」读成「已经在训新 FM」，是白皮书自己划掉的未来工作。分数通道若被改（删检测标记），看起来像安全自改进，其实是在改考官。[10.3 可靠性阶梯](../../../10-评测与安全边界/10.3-可靠性与独立监督/10.3-可靠性与独立监督.md) 要的独立证据，DGM 主实验并不提供：SWE-bench 既是训练信号也是汇报数字。
...
STOP / Gödel Agent / ADAS 的对照见 [9.1.4 STOP](../9.1.4-STOP-自教优化器/9.1.4-STOP-自教优化器.md)、[9.1.5 Gödel Agent](../9.1.5-Godel-Agent-自指运行时/9.1.5-Godel-Agent-自指运行时.md) 与 [9.1.6 ADAS](../9.1.6-ADAS-Meta-Agent-Search/9.1.6-ADAS-Meta-Agent-Search.md)；验证门谁说了算看 [9.1.1 Argus](../9.1.1-Argus-Verification-Gated/9.1.1-Argus-Verification-Gated.md)。产物层回 [9.3.2 AlphaEvolve](../../9.3-科学发现与产物搜索/9.3.2-AlphaEvolve-进化编码智能体/9.3.2-AlphaEvolve-进化编码智能体.md)。模仿信号还是可验证奖励，回 [1.3 模仿与 RLVR](../../../01-研究框架与术语/1.3-模仿学习与RLVR/1.3-模仿学习与RLVR.md)——DGM 冻 $\theta$，连 RLVR 都不是。
...
8. 本花园：[1.1 术语](../../../01-研究框架与术语/1.1-RSI-术语辨析/1.1-RSI-术语辨析.md)；[9.3.2 AlphaEvolve](../../9.3-科学发现与产物搜索/9.3.2-AlphaEvolve-进化编码智能体/9.3.2-AlphaEvolve-进化编码智能体.md)；[10.3 可靠性](../../../10-评测与安全边界/10.3-可靠性与独立监督/10.3-可靠性与独立监督.md)。
