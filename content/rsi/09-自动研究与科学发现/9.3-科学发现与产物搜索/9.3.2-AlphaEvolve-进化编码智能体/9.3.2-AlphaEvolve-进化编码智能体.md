---
title: AlphaEvolve：进化编码智能体改算法不改自己
date: 2026-08-31
as_of: 2026-08-31
category: 自动研究与科学发现
published: true
...
本篇是 Artifact 层的样板。三层坐标在 [1.2 Model–Harness–Artifact](../../../01-研究框架与术语/1.2-Model-Harness-Artifact/1.2-Model-Harness-Artifact.md)；能不能叫 RSI，用 [1.1 术语](../../../01-研究框架与术语/1.1-RSI-术语辨析/1.1-RSI-术语辨析.md) 的式 (1)(2)。**不是** AlphaTensor 那种专打矩阵乘的强化学习，**不是** [9.1.3 DGM](../../9.1-自改代码与元智能体/9.1.3-DGM-达尔文哥德尔机/9.1.3-DGM-达尔文哥德尔机.md) 那种改自己脚手架的编码 Agent，也**不是**把报告里的 OPD 再推一遍。一手：Novikov, Vũ, Eisenberger 等，Google DeepMind 白皮书 [arXiv:2506.13131](https://arxiv.org/abs/2506.13131)；官方博客 [2025-05-14](https://deepmind.google/blog/alphaevolve-a-gemini-powered-coding-agent-for-designing-advanced-algorithms/)。
...
2023 年同一条线上已经有 FunSearch（Romera-Paredes 等，*Nature*；机制精读见 [9.3.3 FunSearch](../9.3.3-FunSearch-函数空间搜索/9.3.3-FunSearch-函数空间搜索.md)）：冻结的代码 LLM 当变异算子，评估器挡幻觉，在函数空间里搜。cap set 这类极值组合问题靠它刷新过已知构造（$n=8$ 时 512）。骨架仍是「人写评估 + 人写种子函数 + 岛模型保多样性」。
...
| 系统 | 改什么 | 改进器在哪 | 本花园 |
|------|--------|------------|--------|
| [FunSearch](../9.3.3-FunSearch-函数空间搜索/9.3.3-FunSearch-函数空间搜索.md) | 单个短函数 | 冻结小代码 LLM + $h$ | Artifact 前身 |
| AlphaTensor | 矩阵乘张量分解 | 专精 RL 策略 | 专才发现，不是通用编码 Agent |
| AlphaEvolve | 整文件算法 / kernel / RTL / IR | 冻结 Gemini 2.0 + 人写 $h$ | Artifact；可反哺训练栈 |
| SPIN / Self-Rewarding | 权重 | 损失与数据锚大多固定 | Model 层，见第 02 章 |
| DGM / STOP / Gödel Agent | Agent 自己的代码 | 基座可冻结，改的是 harness | 第 9.1 节；弱 RSI 候选 |
...
若继续读自动科研闭环，看 [9.2.1 · Polaris](../../9.2-自动研究系统/9.2.1-Polaris-科研智能体/9.2.1-Polaris-科研智能体.md)（文献→实验→写作，默认仍是 Artifact）。前身机制见 [9.3.3 · FunSearch](../9.3.3-FunSearch-函数空间搜索/9.3.3-FunSearch-函数空间搜索.md)。若问「改自己代码的编码 Agent」，转到 [9.1 · 自改代码与元智能体](../../9.1-自改代码与元智能体/9.1-自改代码与元智能体.md)。行业通稿不作为数字来源，本篇只采用白皮书与官方材料。
...
6. 本花园：[1.2 三层](../../../01-研究框架与术语/1.2-Model-Harness-Artifact/1.2-Model-Harness-Artifact.md)；[1.1 术语](../../../01-研究框架与术语/1.1-RSI-术语辨析/1.1-RSI-术语辨析.md)。数学构造：[Colab](https://colab.research.google.com/github/google-deepmind/alphaevolve_results/blob/master/mathematical_results.ipynb)。
