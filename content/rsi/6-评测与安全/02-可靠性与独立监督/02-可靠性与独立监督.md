...
- **L2**：改脚手架。技能包、记忆策略、Argus 的 $H_t$ 都在这里。[ExpeL](../../3-Harness层-Agent运行时/32-ExpeL-跨题经验洞察/32-ExpeL-跨题经验洞察.md) 的洞察清单跨题还在，抽取配方冻着，也停在这一档。[Dynamic Cheatsheet](../../3-Harness层-Agent运行时/33-Dynamic-Cheatsheet-测试时备忘录/33-Dynamic-Cheatsheet-测试时备忘录.md) 的 \(M\) 跨查询还在，Gen / Cur 提示冻着，同样停在这一档。[BoT](../../3-Harness层-Agent运行时/34-BoT-思维模板缓冲/34-BoT-思维模板缓冲.md) 的 meta-buffer 跨题还在，蒸馏提示冻着，也停在这一档。[AWM](../../3-Harness层-Agent运行时/35-AWM-工作流记忆/35-AWM-工作流记忆.md) 的工作流跨题还在，诱导提示冻着，在线入库走 LM 裁判，同样停在这一档。特征失败是过拟合某一套 benchmark × 记忆 × 工作流的交互。验收必须是匹配预算的新任务对比，加上可测回滚。
