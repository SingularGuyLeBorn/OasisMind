...
> 图 1：可靠性阶梯。左低右高为 L0→L4；右侧虚线墙外是证据源与验收门。（自绘，对应综述 Figure 8 的配对思想）
...
> 图 2：EEE 圈的墙。Propose 不得改 Ev 与 G。（自绘，对应综述 Figure 3 的分工）
...
- 删除「Anthropic 2028 前 RSI 概率 60%」的确定表述：官方 Institute 文无此数字；二手博客转述 Jack Clark 个人概率，不升格为实验室结论。
- 将「OpenAI 2028-03 全自动研究员」改为「实验室公开时间表，以访谈 / 报道原文为准」，避免把媒体日期写成事实源。
- L0–L4 表与四条对策仍成立；本版按混元综述 PDF / 项目页补上「更新边界」「development evidence」「RSI 结构前沿在 L3」这些旧摘要没有展开的机制。

下一篇同章：[RSIBench](../01-RSIBench-Data/01-RSIBench-Data.md)（若写评测集）。术语回 [01](../../1-坐标系与术语/01-RSI-术语辨析/01-RSI-术语辨析.md)。
...
4. 库内机制互参：Argus、GPT-Red、AAR 专文（相对链接见 §3）。
