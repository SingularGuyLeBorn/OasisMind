...
- **L0**：改变 $(\text{输出}, \text{轨迹})$，保留的 Agent 状态跨独立任务不变。[Self-Refine](../../3-Harness层-Agent运行时/12-Self-Refine-任务内迭代/12-Self-Refine-任务内迭代.md)、[ReAct](../../3-Harness层-Agent运行时/29-ReAct-推理与动作/29-ReAct-推理与动作.md)、[CRITIC](../../3-Harness层-Agent运行时/13-CRITIC-工具交互批评/13-CRITIC-工具交互批评.md)（有工具仍不跨题保持）、[TextGrad](../../3-Harness层-Agent运行时/14-TextGrad-文本梯度/14-TextGrad-文本梯度.md) 的实例优化、[ToT](../../3-Harness层-Agent运行时/27-ToT-本题推理树/27-ToT-本题推理树.md)、[RAP](../../3-Harness层-Agent运行时/30-RAP-世界模型规划/30-RAP-世界模型规划.md)、[LATS](../../3-Harness层-Agent运行时/28-LATS-Agent树搜/28-LATS-Agent树搜.md)、[GoT](../../3-Harness层-Agent运行时/31-GoT-思维图聚合/31-GoT-思维图聚合.md)、链式验证大多停在这里。危害通常随 episode 丢弃；好处也无法积累。TextGrad 的提示优化可以留下指令，改进器仍在墙外。
...
- **L2**：改脚手架。技能包、记忆策略、Argus 的 $H_t$ 都在这里。[ExpeL](../../3-Harness层-Agent运行时/32-ExpeL-跨题经验洞察/32-ExpeL-跨题经验洞察.md) 的洞察清单跨题还在，抽取配方冻着，也停在这一档。[Dynamic Cheatsheet](../../3-Harness层-Agent运行时/33-Dynamic-Cheatsheet-测试时备忘录/33-Dynamic-Cheatsheet-测试时备忘录.md) 的 \(M\) 跨查询还在，Gen / Cur 提示冻着，同样停在这一档。[BoT](../../3-Harness层-Agent运行时/34-BoT-思维模板缓冲/34-BoT-思维模板缓冲.md) 的 meta-buffer 跨题还在，蒸馏提示冻着，也停在这一档。[AWM](../../3-Harness层-Agent运行时/35-AWM-工作流记忆/35-AWM-工作流记忆.md) 的工作流跨题还在，诱导提示冻着，在线入库走 LM 裁判，同样停在这一档。[MemGPT](../../3-Harness层-Agent运行时/36-MemGPT-操作系统式记忆/36-MemGPT-操作系统式记忆.md) 的分层外存跨会话还在，换页政策冻着，DMR 93.4 是慷慨裁判，也停在这一档。[A-Mem](../../3-Harness层-Agent运行时/37-A-Mem-卡片盒记忆/37-A-Mem-卡片盒记忆.md) 的笔记库跨会话还在，三份提示冻着，GPT 多跳 27.02 不是两倍，同样停在这一档。[HippoRAG](../../3-Harness层-Agent运行时/38-HippoRAG-海马索引检索/38-HippoRAG-海马索引检索.md) 的知识图跨查询还在，OpenIE 与阻尼冻着，2Wiki 的 20.9 是百分点，也停在这一档。[ChatDB](../../3-Harness层-Agent运行时/39-ChatDB-符号SQL记忆/39-ChatDB-符号SQL记忆.md) 的 SQL 账本跨记录还在，生成 SQL 的提示冻着，82% 是 41/50 合成店账，同样停在这一档。[MemoryBank](../../3-Harness层-Agent运行时/40-MemoryBank-遗忘曲线记忆/40-MemoryBank-遗忘曲线记忆.md) 的检索库跨天还在，\(R=e^{-t/S}\) 冻着，0.716 是 97 题人打分，也停在这一档。[ReadAgent](../../3-Harness层-Agent运行时/41-ReadAgent-gist分页记忆/41-ReadAgent-gist分页记忆.md) 的文档 gist 跨题还在，分页提示冻着，20× 是终查询压缩率，同样停在这一档。[LATM](../../3-Harness层-Agent运行时/42-LATM-函数缓存造工具/42-LATM-函数缓存造工具.md) 的函数缓存跨实例还在，造工具提示冻着，中国剩余 0.0→100.0 是百分点，也停在这一档。[AFlow](../../3-Harness层-Agent运行时/43-AFlow-工作流MCTS/43-AFlow-工作流MCTS.md) 的工作流跨题还在，Claude 与式 (3) 冻着，80.3 是六列均分，4.55% 是成本比，同样停在这一档。[GPTSwarm](../../3-Harness层-Agent运行时/44-GPTSwarm-通信图边概率/44-GPTSwarm-通信图边概率.md) 的边概率跨题还在，REINFORCE 与 DAG 约束冻着，90.2% 是相对涨幅且 GAIA 主表未跑边优化，也停在这一档。[ScoreFlow](../../3-Harness层-Agent运行时/45-ScoreFlow-Score-DPO工作流/45-ScoreFlow-Score-DPO工作流.md) 的生成器 LoRA 跨题还在，Score-DPO 与算子集冻着，8.2% 是百分点差，同样停在这一档。特征失败是过拟合某一套 benchmark × 记忆 × 工作流的交互。验收必须是匹配预算的新任务对比，加上可测回滚。
...
system card 上的 self-improvement / AI R&D 是**能力阈值**（离全自动研发有多近），和本篇阶梯不是同一把尺。前者可以很高而验收仍是 L0 的自我表演；后者可以在很浅的 L1 上已经把 held-out 做严。不要用 GPT-5.6 的能力项给混元 L3 背书。判定见 [04 System Card](../04-System-Card-RSI/04-System-Card-RSI.md)，型号表回 llm-guide 第 14 章。

同章：[RSIBench](../01-RSIBench-Data/01-RSIBench-Data.md) 冻栈只改数据；[SEAGym](../03-SEAGym-Harness评测环境/03-SEAGym-Harness评测环境.md) 冻 $M$ 只改 $H$。术语回 [01](../../1-坐标系与术语/01-RSI-术语辨析/01-RSI-术语辨析.md)。实验室通稿里的时间表以 §4 原文为准，不把媒体日期写成里程碑。
