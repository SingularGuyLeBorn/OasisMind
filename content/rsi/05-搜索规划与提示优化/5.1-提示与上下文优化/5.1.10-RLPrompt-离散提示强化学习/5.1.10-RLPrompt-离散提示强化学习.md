...
# RLPrompt：乱码提示交卷，策略网络扔掉

[GrIPS](../5.1.8-GrIPS-短语级编辑搜索/5.1.8-GrIPS-短语级编辑搜索.md) 在人写说明书上爬山。[TEMPERA](../5.1.9-TEMPERA-测试时提示编辑/5.1.9-TEMPERA-测试时提示编辑.md) 把同类手术交给按查询的 PPO，还要 RoBERTa 隐状态。RLPrompt 反过来：不给人初稿，从词表一个 token 一个 token **生成**离散提示 \(z\in\mathcal{V}^T\)。任务模型冻着当黑盒环境，可走 API。策略是冻住的 distilGPT-2 上面加一层可训 MLP。训完把 MLP 扔掉，交卷只剩那串字。RoBERTa-large、每类 16 条，5 token 行 SST-2 **92.5 (0.8)**，Yelp P. **95.1 (1.0)**，七套均 **75.8**。TEMPERA Table 2 里的 RLPrompt 列是 SST-2 **90.1**、Yelp **93.9**，贴着本篇 **2 token** 行（90.3 / 94.1），**不是** 5 token 主行。禁止用 90.1 改 92.5。
...
少样本分类和可控生成都想冻住大模型，只换前面那截字。软提示好训：向量能梯度。坏处也硬：人读不懂、换一只 LM 对不上隐空间、API 不给梯度。离散提示是词表里的 token，能给人看、能跨模型粘贴。搜索空间却是 \(\mathcal{O}(|\mathcal{V}|^T)\)。前人两条路：释义或枚举再挑（Jiang、Gao、GrIPS），或者用梯度近似改 token（[AutoPrompt](../5.1.11-AutoPrompt-梯度引导触发词/5.1.11-AutoPrompt-梯度引导触发词.md)）。前者不系统，后者仍要内部状态，训练还不稳。
...
和 [TEMPERA](../5.1.9-TEMPERA-测试时提示编辑/5.1.9-TEMPERA-测试时提示编辑.md) 钉死。那边编辑人写初稿、PPO、要隐状态、测试时按查询改 \(T=8\) 步、题做完扔 \(p_T\)。这边从词表生成、SQL、任务 LM 可黑盒、分类查询无关、交卷留字符串。邻居表上的 90.1 对 91.9，不要改本篇 92.5。和 [GrIPS](../5.1.8-GrIPS-短语级编辑搜索/5.1.8-GrIPS-短语级编辑搜索.md) 钉死。那边人写说明书、四种短语手术、贪心、InstructGPT 上 +4.29；这边无初稿、常是乱码、RoBERTa 少样本均 75.8 对同表 GrIPS 69.4。同表 GrIPS 不能回写 Prasad。和 [APE](../5.1.5-APE-自动提示工程师/5.1.5-APE-自动提示工程师.md) 钉死：APE 用大模型整段提案，默认不迭代，骨干 InstructGPT，GSM8K 43.0 和本篇 RoBERTa 分类不是一列。和 [OPRO](../5.1.3-OPRO-元提示优化/5.1.3-OPRO-元提示优化.md)、[EvoPrompt](../5.1.4-EvoPrompt-进化算子提示/5.1.4-EvoPrompt-进化算子提示.md) 钉死：历史分数进元提示、GA/DE 说明书冻着，都不是逐步 RL 出 token。和 [ProTeGi](../5.1.7-ProTeGi-文本梯度束搜索/5.1.7-ProTeGi-文本梯度束搜索.md) 钉死：那边错题进批评模板；这边没有「哪里错了」的句子，只有间隔和 SQL。和 [Self-Refine](../../../04-反思批评与反馈优化/4.1-Self-Refine-任务内迭代/4.1-Self-Refine-任务内迭代.md) 钉死：那边改本题答案 \(y\)，跨题清空；这边改 \(z\)，分类测试集共用。[AutoPrompt](../5.1.11-AutoPrompt-梯度引导触发词/5.1.11-AutoPrompt-梯度引导触发词.md) 要梯度改离散 token，本篇故意绕开。Black-Box Tuning 混软硬提示，主表 74.7 是最近的连续侧对手，仍略低于 5 token 行。Figure 2 把测试准确率对训练步画在一起：本方法收敛步数和 BB Tuning 相近，收敛后最差的那些提示，均分仍贴着 BB Tuning。作者拿来挡「RL 更慢」。梯度免费、步数相当，赢在终点略高，不赢在更少的黑盒查询。查询预算两篇没有钉成同一列。
...
同层：[5.1.11 AutoPrompt](../5.1.11-AutoPrompt-梯度引导触发词/5.1.11-AutoPrompt-梯度引导触发词.md)、[5.1.9 TEMPERA](../5.1.9-TEMPERA-测试时提示编辑/5.1.9-TEMPERA-测试时提示编辑.md)、[5.1.8 GrIPS](../5.1.8-GrIPS-短语级编辑搜索/5.1.8-GrIPS-短语级编辑搜索.md)、[5.1.5 APE](../5.1.5-APE-自动提示工程师/5.1.5-APE-自动提示工程师.md)、[5.1.7 ProTeGi](../5.1.7-ProTeGi-文本梯度束搜索/5.1.7-ProTeGi-文本梯度束搜索.md)、[5.1.3 OPRO](../5.1.3-OPRO-元提示优化/5.1.3-OPRO-元提示优化.md)、[5.1.4 EvoPrompt](../5.1.4-EvoPrompt-进化算子提示/5.1.4-EvoPrompt-进化算子提示.md)、[4.1 Self-Refine](../../../04-反思批评与反馈优化/4.1-Self-Refine-任务内迭代/4.1-Self-Refine-任务内迭代.md)。综述里的生成派：[1.4](../../../01-研究框架与术语/1.4-自进化Agent综述/1.4-自进化Agent综述.md)。
...
4. 本花园：[TEMPERA](../5.1.9-TEMPERA-测试时提示编辑/5.1.9-TEMPERA-测试时提示编辑.md)；[GrIPS](../5.1.8-GrIPS-短语级编辑搜索/5.1.8-GrIPS-短语级编辑搜索.md)；[APE](../5.1.5-APE-自动提示工程师/5.1.5-APE-自动提示工程师.md)。
