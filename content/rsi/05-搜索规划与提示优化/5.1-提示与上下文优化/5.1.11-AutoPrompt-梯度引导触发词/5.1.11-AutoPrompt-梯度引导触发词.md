...
和 [RLPrompt](../5.1.10-RLPrompt-离散提示强化学习/5.1.10-RLPrompt-离散提示强化学习.md) 钉死。那边 SQL 生成、可黑盒、少样本 16/类、5 token SST-2 92.5；这边 HotFlip 替换、要梯度、全量 SST-2、测试 91.4。两格都叫 RoBERTa、都叫 SST-2，训练条数不是一回事。邻居少样本表上的 AutoPrompt 均 56.7，是 Deng 等的复现，不要写回 Table 1。和 [TEMPERA](../5.1.9-TEMPERA-测试时提示编辑/5.1.9-TEMPERA-测试时提示编辑.md) 钉死：那边 PPO 编辑人写初稿、按查询、隐状态当状态；这边触发词查询无关、梯度在嵌入上。TEMPERA 表 SST-2 91.9 对的是邻居 RLPrompt 90.1，不是本篇 91.4。和 [GrIPS](../5.1.8-GrIPS-短语级编辑搜索/5.1.8-GrIPS-短语级编辑搜索.md) 钉死：短语手术无梯度，InstructGPT 上 +4.29；本篇有梯度，骨干是 BERT/RoBERTa。GrIPS 写「Prefix 上界住 AutoPrompt」是传递不等式，不是已经跑过 Shin 等的表。和 [APE](../5.1.5-APE-自动提示工程师/5.1.5-APE-自动提示工程师.md) 钉死：APE 用大模型整段写人话指令；这边触发词常不通。和 [ProTeGi](../5.1.7-ProTeGi-文本梯度束搜索/5.1.7-ProTeGi-文本梯度束搜索.md)、[TextGrad](../../../04-反思批评与反馈优化/4.3-TextGrad-文本梯度/4.3-TextGrad-文本梯度.md) 钉死：文本梯度是自然语言批评句；这边梯度是嵌入点积，没有「哪里错了」的句子。和 [Self-Refine](../../../04-反思批评与反馈优化/4.1-Self-Refine-任务内迭代/4.1-Self-Refine-任务内迭代.md) 钉死：那边改本题答案，跨题清空；这边改共用触发词。
...
同层：[5.1.12 PromptAgent](../5.1.12-PromptAgent-MCTS提示规划/5.1.12-PromptAgent-MCTS提示规划.md)、[5.1.10 RLPrompt](../5.1.10-RLPrompt-离散提示强化学习/5.1.10-RLPrompt-离散提示强化学习.md)、[5.1.9 TEMPERA](../5.1.9-TEMPERA-测试时提示编辑/5.1.9-TEMPERA-测试时提示编辑.md)、[5.1.8 GrIPS](../5.1.8-GrIPS-短语级编辑搜索/5.1.8-GrIPS-短语级编辑搜索.md)、[5.1.5 APE](../5.1.5-APE-自动提示工程师/5.1.5-APE-自动提示工程师.md)、[5.1.7 ProTeGi](../5.1.7-ProTeGi-文本梯度束搜索/5.1.7-ProTeGi-文本梯度束搜索.md)、[4.3 TextGrad](../../../04-反思批评与反馈优化/4.3-TextGrad-文本梯度/4.3-TextGrad-文本梯度.md)。综述里的离散梯度搜：[1.4](../../../01-研究框架与术语/1.4-自进化Agent综述/1.4-自进化Agent综述.md)。
...
4. 本花园：[RLPrompt](../5.1.10-RLPrompt-离散提示强化学习/5.1.10-RLPrompt-离散提示强化学习.md)；[GrIPS](../5.1.8-GrIPS-短语级编辑搜索/5.1.8-GrIPS-短语级编辑搜索.md)；[TEMPERA](../5.1.9-TEMPERA-测试时提示编辑/5.1.9-TEMPERA-测试时提示编辑.md)。
