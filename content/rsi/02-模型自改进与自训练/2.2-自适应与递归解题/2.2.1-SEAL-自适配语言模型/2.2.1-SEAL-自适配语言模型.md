...
SEAL 的假设是：模型已经「读得懂」这段 $C$（SQuAD 段落在上下文里能答对），缺的是**把读懂的东西变成适合梯度更新的文本，并选对更新超参**。这和 [术语篇](../../../01-研究框架与术语/1.1-RSI-术语辨析/1.1-RSI-术语辨析.md) 里的 TTT 轴要对上：内环就是一次测试时训练；外环另外学「这一次 TTT 该怎么配」。
...
$S$ 取当前权重 $\theta$。内环式 (1) 之后 $S'=\theta'$。外环把「如何生成 SE」也写进 $\theta$，所以 **self-edit 策略住在权重里**，下一轮 ReST-EM 用的已经是更新过的写手。按 [1.1 术语](../../../01-研究框架与术语/1.1-RSI-术语辨析/1.1-RSI-术语辨析.md) 式 (1)，这是合格的 Model 层自改进；比 SPIN 更像「改进器在变」，因为 SPIN 的损失形状固定、变的只是对手权重，SEAL 变的是「写什么样的训练数据」。
...
**验收必须绑着 $\tau$。** 每段 $C$ 都要配好现成问答或 held-out 测试格，否则算不出 $r$。外环因此走不上无标签语料。作者设想让模型自己出题当即时监督——那会把考官部分地搬进 $S$，[10.3 可靠性](../../../10-评测与安全边界/10.3-可靠性与独立监督/10.3-可靠性与独立监督.md) 所要求的独立证据就更弱。本文没有跑这条。
...
同层：[SPIN](../../2.1-自生成数据与自奖励/2.1.1-SPIN-自对弈微调/2.1.1-SPIN-自对弈微调.md) 是分布匹配；[Self-Rewarding 家族](../../2.1-自生成数据与自奖励/2.1.2-Self-Rewarding-家族/2.1.2-Self-Rewarding-家族.md) 是自打分。Harness 对照：[9.1.3 DGM](../../../09-自动研究与科学发现/9.1-自改代码与元智能体/9.1.3-DGM-达尔文哥德尔机/9.1.3-DGM-达尔文哥德尔机.md)。Artifact 对照：[9.3.2 AlphaEvolve](../../../09-自动研究与科学发现/9.3-科学发现与产物搜索/9.3.2-AlphaEvolve-进化编码智能体/9.3.2-AlphaEvolve-进化编码智能体.md)。
...
7. 本花园：[1.1 术语](../../../01-研究框架与术语/1.1-RSI-术语辨析/1.1-RSI-术语辨析.md)；[2.1.1 SPIN](../../2.1-自生成数据与自奖励/2.1.1-SPIN-自对弈微调/2.1.1-SPIN-自对弈微调.md)；[9.1.3 DGM](../../../09-自动研究与科学发现/9.1-自改代码与元智能体/9.1.3-DGM-达尔文哥德尔机/9.1.3-DGM-达尔文哥德尔机.md)。
