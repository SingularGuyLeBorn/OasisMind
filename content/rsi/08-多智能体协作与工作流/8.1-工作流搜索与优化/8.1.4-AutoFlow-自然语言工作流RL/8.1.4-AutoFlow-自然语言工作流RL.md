---
title: AutoFlow：自然语言工作流会搜，解释器冻着
date: 2026-08-31
as_of: 2026-08-31
category: 多智能体协作与工作流
published: true
...
解释器逐步执行，每一步内部又是四段，全是 CoRE 系统里写死的。先从记忆里取这一步可能用到的信息；再和指令拼成提示，让 LLM 出初步回应；再看要不要调外部工具，要就选工具名和参数，结果写回记忆；最后根据本步输出决定下一步。工具调用发生在解释器里，不发生在生成器里。生成器只负责吐整份工作流文本。OpenAGI 的观察可以是图，所以他们没把 [ReAct](../../../05-搜索规划与提示优化/5.2-推理搜索与规划/5.2.3-ReAct-推理与动作/5.2.3-ReAct-推理与动作.md) 收进基线：ReAct 要文本观察。CoT 基线是用 DSPy 接到 OpenAGI 上跑的，不是另写一套逐步提示。Program-of-Thought 在这个平台上吐不出可执行代码，也没进表。不要用「没比 ReAct」听成已经赢了 ReAct。是任务形态不接。
...
同一句「自动生成 Agent 工作流」，至少分五截。提示优化把图钉死。ADAS 线性搜代码。AFlow 用 MCTS 加算子。ScoreFlow 用 Score-DPO 训 Python 生成器。AutoFlow 用 CoRE 字符串加 REINFORCE 或分数提示。五截不要收成「都已经是 RSI」。[MAS-GPT](../../8.2-多智能体拓扑与协作/8.2.2-MAS-GPT-一次前向吐MAS/8.2.2-MAS-GPT-一次前向吐MAS.md) 一次前向吐可执行 MAS 代码，Llama-3-70B 八列均分 65.47，3.89 是百分点。[G-Designer](../../8.2-多智能体拓扑与协作/8.2.3-G-Designer-任务自适应通信图/8.2.3-G-Designer-任务自适应通信图.md) 按任务生成图，MMLU 84.50 / HumanEval 89.90。[AgentPrune](../../8.2-多智能体拓扑与协作/8.2.4-AgentPrune-时空图剪边/8.2.4-AgentPrune-时空图剪边.md) 剪边省 token，均分 89.72。FlowMind 也是 LLM 出工作流，相关工作点了名，主表没有它。不要和 AFlow 收成同一张 80.3。
...
同层：[8.1.1 AFlow](../8.1.1-AFlow-工作流MCTS/8.1.1-AFlow-工作流MCTS.md)、[8.1.2 ScoreFlow](../8.1.2-ScoreFlow-Score-DPO工作流/8.1.2-ScoreFlow-Score-DPO工作流.md)、[8.1.3 MASS](../8.1.3-MASS-提示拓扑分阶段/8.1.3-MASS-提示拓扑分阶段.md)、[8.2.2 MAS-GPT](../../8.2-多智能体拓扑与协作/8.2.2-MAS-GPT-一次前向吐MAS/8.2.2-MAS-GPT-一次前向吐MAS.md)、[8.2.3 G-Designer](../../8.2-多智能体拓扑与协作/8.2.3-G-Designer-任务自适应通信图/8.2.3-G-Designer-任务自适应通信图.md)、[8.2.4 AgentPrune](../../8.2-多智能体拓扑与协作/8.2.4-AgentPrune-时空图剪边/8.2.4-AgentPrune-时空图剪边.md)、[8.2.5 MaAS](../../8.2-多智能体拓扑与协作/8.2.5-MaAS-Agent超网/8.2.5-MaAS-Agent超网.md)、[8.2.6 ANN](../../8.2-多智能体拓扑与协作/8.2.6-ANN-层状文本反传/8.2.6-ANN-层状文本反传.md)、[9.1.6 ADAS](../../../09-自动研究与科学发现/9.1-自改代码与元智能体/9.1.6-ADAS-Meta-Agent-Search/9.1.6-ADAS-Meta-Agent-Search.md)、[5.1.3 OPRO](../../../05-搜索规划与提示优化/5.1-提示与上下文优化/5.1.3-OPRO-元提示优化/5.1.3-OPRO-元提示优化.md)、[6.2.1 LATM](../../../06-工具学习与程序编排/6.2-工具创建与文档优化/6.2.1-LATM-函数缓存造工具/6.2.1-LATM-函数缓存造工具.md)、[5.2.3 ReAct](../../../05-搜索规划与提示优化/5.2-推理搜索与规划/5.2.3-ReAct-推理与动作/5.2.3-ReAct-推理与动作.md)、[8.2.1 GPTSwarm](../../8.2-多智能体拓扑与协作/8.2.1-GPTSwarm-通信图边概率/8.2.1-GPTSwarm-通信图边概率.md)、[9.1.5 Gödel Agent](../../../09-自动研究与科学发现/9.1-自改代码与元智能体/9.1.5-Godel-Agent-自指运行时/9.1.5-Godel-Agent-自指运行时.md)、[9.1.1 Argus](../../../09-自动研究与科学发现/9.1-自改代码与元智能体/9.1.1-Argus-Verification-Gated/9.1.1-Argus-Verification-Gated.md)。台阶：[10.3 可靠性](../../../10-评测与安全边界/10.3-可靠性与独立监督/10.3-可靠性与独立监督.md)。术语：[1.1](../../../01-研究框架与术语/1.1-RSI-术语辨析/1.1-RSI-术语辨析.md)。
...
5. 本花园：[AFlow](../8.1.1-AFlow-工作流MCTS/8.1.1-AFlow-工作流MCTS.md)；[ScoreFlow](../8.1.2-ScoreFlow-Score-DPO工作流/8.1.2-ScoreFlow-Score-DPO工作流.md)。AFlow 原文均分 80.3 以专文为准，不要和本表 0.36 / 0.65 横加。
