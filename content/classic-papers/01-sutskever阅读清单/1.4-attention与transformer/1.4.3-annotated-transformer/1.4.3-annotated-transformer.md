...
published: true
excerpt: >-
  本站把路线 `1.4.2` Attention Is All You Need 与 `1.4.3` The Annotated Transformer 配成「论文原文 +
  逐行代码注解」学习单元. 2018 年 4 月, Harvard NLP 的 Alexander Rush 拉上 OpenNMT 的 Vincent
  Nguyen 和 Guillaume Klein, 把论文 8 页正文拆成段落, 每段配上可运行的 PyTorch 代码, 全文本身就是一个
  Jupyter notebook. 400 行库代码, 4 张 V100 上跑到 27000 tokens/s. 本帖不复述路线 `1.4.2` 的
tags:
  - Ilya
  - Transformer
...
> 这是本站学习路线 `1.4.3`, 紧接 `1.4.2` Attention Is All You Need。两篇组成“论文设计 + 注释实现”的配对：前者讲架构，本文把关键模块落成代码。实现规模、框架版本和吞吐数字应以作者页面对应版本为准。

本文以代码为主。注意力公式与架构推导可先阅读路线 [`1.4.2`](../1.4.2-transformer/1.4.2-transformer.md)，本文重点解释实现如何对应论文组件。
...
**读公式的方法**: 两条公式都是路线 `1.4.2` 的原式, 读法也一样. §5 注意力公式按三步读: $QK^T$ 打分, 除 $\sqrt{d_k}$ 再过 softmax 归一化成权重, 乘 $V$ 做加权求和. 本文的重点不在公式本身, 在公式没写的地方: mask 填什么值, dropout 乘在谁身上, 返回几个值. §9 学习率公式读「两条曲线取更小那条」, 升温支和降温支的交点恰好在 warmup.
...
Rush 的回应是一篇把论文段落与可运行 PyTorch 代码交替编排的博客, 全文本身由 Jupyter notebook 生成，可以直接执行. 作者阵容也值得注意. Alexander Rush 领导 Harvard NLP，Vincent Nguyen 和 Guillaume Klein 参与 OpenNMT 开发；OpenNMT 是 2017 年发表的开源神经机器翻译工具包. 因而这份注解来自有实际 NMT 系统经验的研究者，但代码定位仍是便于教学和复现的紧凑实现，不应等同于完整生产系统.
...
这种体裁后来影响深远. 2018 年之后，很多教程与课程沿用它的代码结构，包括 make_model 组装函数、EncoderDecoder 基类和 SublayerConnection 包装. 在成熟框架普及前，这份博客是 PyTorch 学习者理解和复现 Transformer 的常用入口之一.
...
公式本身的逐块拆解在路线 `1.4.2`, 这里只重申读法. $QK^T$ 是打分: 上标 $T$ 是转置 (行列互换, 本文上标 $T$ 只有这一个含义), 乘出来的矩阵第 $(i, j)$ 个元素是 $q_i \cdot k_j$, 即「位置 $i$ 找位置 $j$」的匹配分. 除以 $\sqrt{d_k}$ 把方差压回 1, 再过 softmax 按行归一化成权重. 最后乘 $V$, 每一行输出是所有位置 value 的加权和. 三步全是矩阵乘, 零循环.
...
读法和路线 `1.4.2` 相同: 前面 $d_{\text{model}}^{-0.5}$ 是基准步长 (512 维时约 0.0442), min 里第二条 $step \cdot warmup^{-1.5}$ 是与步数成正比的升温支, 第一条 $step^{-0.5}$ 是与步数平方根成反比的降温支, min 取更小者, 前期走升温线, 后期走降温线, 交点恰好在 $step = warmup$. 代入三个点的数值曲线 (第 2000 步约 $3.5 \times 10^{-4}$, 第 4000 步峰值约 $7.0 \times 10^{-4}$, 第 100000 步回落到约 $1.4 \times 10^{-4}$) 见路线 `1.4.2` §7.
...
局限也要说清楚. 这份代码是教学版, 不是生产版: 没有 KV cache, 没有 beam search 实现, 多 GPU 方案基于旧的 DataParallel, 学习率调度器要自己包. 生产使用应优先选择持续维护的框架或官方实现；但想理解 mask 为什么填 -1e9, embedding 为什么乘 √d_model, norm 为什么可以放前面, 这份紧凑注解仍然是很好的学习材料.

放回本站学习路线, 路线 `1.4.2` 和路线 `1.4.3` 构成「原文 + 注解」配对. 这个配对本身传递了一个信号: 理解一个架构, 读论文和读代码是两件不同的事, 都值得做. 论文给你公式和动机, 代码给你维度和数值. 两者之间的缝隙里, 住着复现失败的全部原因.

## 13. 扩展阅读

- Vaswani et al., "Attention Is All You Need", NeurIPS 2017. https://arxiv.org/abs/1706.03762 (本系列路线 `1.4.2`)
