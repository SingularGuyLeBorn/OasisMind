...
published: true
excerpt: >-
  本站学习路线 `1.3.2` 不是论文, 是 Christopher Olah 2015 年 8 月的一篇博客, 全世界被引用最多的 LSTM 教程.
...
> 本站学习路线 `1.3.2`. 路线 `1.3.1` Karpathy 的博客展示 RNN 能做出多惊人的效果, 本篇拆开 RNN 内部, 讲清楚 LSTM 凭什么能做到, 路线 `1.3.3` 接着解决 LSTM 的过拟合. 三篇构成本站学习路线里的 RNN 三部曲.
...
| $t$ | 时间步, 序列里的第几个位置 |
| $x_t$ | 第 $t$ 步的输入向量 |
| $h_t$ | 第 $t$ 步的 hidden state, 模块对外的输出, 也是工作记忆. 和路线 `1.3.1` 的 $h_t$ 是同一个东西 |
...
博客末尾的致谢名单值得注意: 作者感谢 Google 的同事 Oriol Vinyals, Greg Corrado, Jon Shlens, Luke Vilnis, 以及 Ilya Sutskever；还感谢了 Dario Amodei 和 Kyunghyun Cho。Cho 是 GRU 的作者, 也是本站学习路线 `1.4.1` attention 论文的通讯作者。这些人物关系能帮助理解研究社区的延续，但不能证明社区重建书目的来源或顺序。

这篇博客的方法论价值, 和它的内容价值至少一样大. Olah 定了一套图例: 每条线携带一整个向量, 黄色方块是可学习的神经网络层, 粉色圆圈是逐点运算 (比如向量加法), 线条合并表示拼接, 线条分叉表示内容被复制送往不同位置. 这套图例做了一件此前没人系统做过的事: 给「把公式翻译成图」这件事建立语法. 公式写成一行, 信息是顺序排列的符号; 画成图, 信息有了拓扑结构, 哪条通路短, 哪条通路长, 梯度在哪条路线上衰减, 一眼可见. 本站学习路线 `1.4.3` The Annotated Transformer 是这个传统的直接继承者.
...
这个预告的命中率是 100%. 本站学习路线 `1.4.1` 就是 Cho 团队的 attention 机翻论文, 路线 `1.4.2` Attention Is All You Need 干脆把 RNN 本体扔掉, 只留 attention. 2015 年时「让每一步看向输入子集」还是一个研究方向, 十年后它是整个大模型时代的主架构. Olah 本人后来也转向可解释性研究, 在 Distill 和 Anthropic 继续写那种「把复杂机制讲穿」的文章, 这篇博客是他方法论的原点.
...
把这篇放回本站学习路线的结构里, 它是 RNN 三部曲的中轴. 路线 `1.3.1` Karpathy 的 The Unreasonable Effectiveness of Recurrent Neural Networks 展示效果: 字符级 RNN 能写出像样的莎士比亚, 代码, 甚至 LaTeX. 路线 `1.3.2` Olah 拆机制: 这些效果背后是 LSTM 的门控记忆. 路线 `1.3.3` Recurrent Neural Network Regularization 解决工程问题: LSTM 这么能拟合, 怎么防止过拟合. 效果, 机制, 正则化, 三篇排成一条完整的技术叙事线.

人物线同样清楚. GRU 的作者 Kyunghyun Cho, 正是路线 `1.4.1` attention 论文的通讯作者；Olah 的致谢里还有 Ilya Sutskever 和 Dario Amodei。本站把这些材料并读，是为了展示门控记忆、注意力与后续研究之间的人物和思想联系，不把致谢关系解释成书目认证。

方法论上, 这篇博客示范了「图解工程」: 用一套严格统一的视觉语法, 把一组 intimidating 的方程变成 approachable 的图. 公式逐行走, 图按模块拆, 每个图只高亮当前讲的那部分, 其余灰掉. 这套手法后来成了技术写作的行业标准, 路线 `1.4.3` The Annotated Transformer 用逐行代码注释加图做同样的事. 对今天写技术内容的人, 这篇博客的教训比 LSTM 本身更耐用: 表达不是研究完成后的包装, 表达本身就是研究能力的一部分.

**三部曲与人物线**

中心节点是本篇, 左接路线 `1.3.1` Karpathy 的效果展示, 右接路线 `1.3.3` 的正则化. 上方一条人物线: Cho 从 GRU 通向路线 `1.4.1` attention, 虚线指向路线 `1.4.2` Transformer.
...
- Cho, K. et al. (2014). Learning Phrase Representations using RNN Encoder-Decoder for Statistical Machine Translation. GRU 原始论文, 作者即本站学习路线 `1.4.1` 通讯作者.
...
- Karpathy, A. (2015). The Unreasonable Effectiveness of Recurrent Neural Networks. 本站学习路线 `1.3.1`, 三部曲前作.
- Zaremba, W., Sutskever, I. & Vinyals, O. (2014). Recurrent Neural Network Regularization. 本站学习路线 `1.3.3`, 三部曲续作.
