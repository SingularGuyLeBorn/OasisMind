...
published: true
excerpt: >-
  本站学习路线 `1.5.5` 是 Google 2017 年的 MPNN 论文. 它有两个身份. 对化学界, 它把 DFT 计算加速 30 万倍, QM9
...
> 原始论文：Gilmer et al., ICML 2017, [arXiv:1704.01212](https://arxiv.org/abs/1704.01212)。

本站学习路线 `1.5.5` 是 Gilmer, Schoenholz, Riley, Vinyals, Dahl 2017 年发在 ICML 的工作. 它有两个身份. 对化学界, 它是一个把 DFT 计算加速 30 万倍的分子性质预测器, 在 QM9 的 13 个性质里有 11 个达到化学精度. 对机器学习界, 它做了一件更持久的事: 把当时文献里至少八个各自为政的图神经网络模型, 统一进一个叫 Message Passing Neural Network (MPNN) 的框架, 给这个领域立了一套沿用至今的记号. 今天 PyTorch Geometric 里 MessagePassing 基类的三个方法 message, aggregate, update, 就是这篇论文三个方程的直接后代.

本文完整展开量子化学背景、统一框架的数学、八个模型的对照、QM9 实验细节、消息传递的盲区，以及它与路线 `1.5.3`、`1.5.4` 的关系。当前库中只保留这一份完整正文。
...
**读出函数.** 基线是 GG-NN 的门控求和. 论文换上 Vinyals et al. 2015 的 set2set: 先对每个 $(h_v^T, x_v)$ 做线性投影, 然后跑 $M$ 步集合编码计算, 产出对节点顺序不变的图级嵌入, 再过一个神经网络出结果. set2set 比纯求和表达力更强, 注意它和本站学习路线 `1.5.2` Order Matters 是同一篇论文的机器, 这次是 Oriol Vinyals 本人把它用在自己的新论文里.
...
## 8. 与路线 `1.5.3`、`1.5.4` 的关系：关系推理的第三种形态

本站 `1.5` 主题组讨论结构化推理，其中 `1.5.3`、`1.5.4`、`1.5.5` 分别从静态对象关系、循环记忆与图消息传递切入，作者网络也存在交叉。

路线 `1.5.3` Relation Network: 对象集合上, 枚举所有对象对, 一次性算完全部关系, 求和出答案. 计算图是全连接, 一步完成, 没有迭代. 它回答「关系该算什么」, 不回答「关系沿什么结构传播」.

路线 `1.5.4` Relational RNN: 把关系计算装进循环记忆, 多头自注意力在记忆槽之间做关系推理, 按时间步迭代. 结构是固定的槽位全连接, 迭代是序列式的.

路线 `1.5.5` MPNN: 关系沿任意图结构的边传播, 每步只和邻居交换消息, 迭代 $T$ 步. 结构是输入给定的, 传播是局部的.

三者拼起来是一个完整的光谱. RN 是「没有结构时的暴力兜底」, RRNN 是「结构是时间时的循环方案」, MPNN 是「结构是图时的标准答案」. 分子, 物理系统, 社交网络这些领域, 结构本身就是输入数据的一部分, 忽略结构用 RN 暴力枚举, 既浪费计算又丢掉先验. MPNN 把「沿边传播」烤进架构, 和路线 `1.5.3` 把「枚举对象对」烤进架构是同一种哲学: 归纳偏置不是累赘, 是数据效率的来源.

还有一层呼应藏在论文结尾. MPNN 全连接图的扩展性问题, 作者开的药方是「给入消息加 attention」. 而路线 `1.5.3` 的结尾, RN 的作者开的药方是「用 attention 过滤不重要的关系」. 两篇论文从不同的痛点出发, 指向同一个机制. 几个月后 Transformer 把这个机制做成通用件. 回头看, 2017 年上半年的这批 Google 论文, 在「两两关系计算加可学习门控」这个构型上反复收敛, Transformer 只是它最成功的一次落地.
...
本站收录这篇，重点在它对既有图神经网络的统一框架。MPNN 论文第 2.1 节把分子机器学习当时的状态与 AlexNet 前后的视觉领域作类比：价值不一定来自发明全部组件，而可能来自在合适基准上把一类方法统一、实现并验证。这个类比来自论文正文，不需要再推断私人书目的选文理由。

## 10. 本站学习路线位置

路线 `1.5.5` 在本站学习路线里的坐标可以这样概括: 承 19 的「对象对关系计算」, 接 20 的「迭代式关系记忆」, 把关系推理推广到任意图结构, 并顺手给一个战国时代立了法. 它同时是这组里最「工程」的一篇: 50 次随机超参搜索, 三种消息函数对比, 五个模型集成, 误差比指标设计, 全部是扎实的 benchmark 方法论. 结构化推理这一组读到这一篇, 主题从「模块长什么样」转向「领域怎么统一, benchmark 怎么打穿」.
...
- Vinyals et al. 2015, Order Matters: Sequence to Sequence for Sets, set2set 读出函数的出处, 也是本站学习路线 `1.5.2`.
