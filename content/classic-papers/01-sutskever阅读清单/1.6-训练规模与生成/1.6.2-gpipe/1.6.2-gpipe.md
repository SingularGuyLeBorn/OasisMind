...
tags:
  ...
  - 经典论文
---
# GPipe：当模型大到单卡装不下，流水线如何接管分布式训练

> 原始论文：Huang et al., 2018, [arXiv:1811.06965](https://arxiv.org/abs/1811.06965)。
...
## 12. 在本站学习路线中的位置: 从算法篇到系统篇的枢纽

本站路线此前的文章几乎全是算法和理论. 复杂度, 信息论, 架构, 注意力, 优化. 路线 `1.6.2` 突然跳进系统层, 这个转折本身就是信号.

本站学习路线的内在线索很连贯. 路线 `1.4.2` Transformer 交付了基础架构；BERT 虽不在本站这 27 篇专文中，却证明了 Transformer 扩展到大规模预训练的价值。问题随后转向工程侧: 模型再大一个数量级, 一台机器放不下了, 怎么办. GPipe 回答的就是这个问题, 答案是把模型分区, 用微批流水线提高设备利用率, 用重计算降低激活显存. 路线 `1.6.4` Scaling Laws 接着讨论规模投入与损失下降的经验关系。

**GPipe 与本站学习路线前后篇目的连接**

路线 `1.6.2` 讨论怎样把大模型装进多台加速器；路线 `1.6.4` 再讨论增加参数、数据与计算量是否值得。Transformer 位于路线 `1.4.2`，BERT 不在本站这组 27 篇正文中。后续工程脉络可继续阅读 PipeDream、Megatron 的张量并行与 DeepSpeed ZeRO；它们共同组成今天常说的数据、张量、流水线等并行维度。
