...
[采样与调度](../02-数学与生成机制/2.6-采样后验调度与重掩.md) 把步数写成测试时的旋钮：切得越细，离散化误差越小，前向越贵。[Score entropy](../02-数学与生成机制/2.5-score-entropy与离散流匹配/2.5.2-score-entropy.md) 把离散反向写成状态比 $p_t(y)/p_t(x)$。[ParallelBench](../07-控制、评测与选型/7.3-parallelbench与并行性评测.md) 问的是数据本身能不能并行。[离散 copula](../04-联合依赖与结构设计/4.3-因子化障碍与联合建模/4.3.1-离散copula.md) 把因子化反向在 ELBO 里留下的总相关写成 $\mathrm{D}_{\mathrm{TC}}$。Dmitriev、Huang、Wei 的 [*Provably adaptive sampling with uniform and remasking discrete diffusion models*](https://arxiv.org/abs/2608.23554) 问的是另一件事：均匀前向配上常见的 $\tau$-leaping，已有下界随环境维 $d$ 线性涨。这线性是前向过程固有的，还是采样器写错了？
...
[提交之后还能不能改](./6.2-纠错重掩与动态调度.md) 写 ReMDM、GIDD、低置信 remask。本篇 remasking 是前向过程：坐标会经过 $\mathrm{REMASK}$ 辅助态，反向才有机会改字。$p_M=0.5$ 是 Figure 1 的设定，不是 LLaDA 推理时的置信阈值 $0.9$。低置信 remask 是冻结规划器；这里的 remasking 是 $Q_t$ 的定义。ReMDM 改的是反向后验、可以套预训练权重；GIDD 训练时就见乱词。本篇既不改后验公式，也不改训练目标，只改给定分数之后怎么在区间上模拟 CTMC。三篇都出现「改字」，改的层分别是采样启发式、前向 $Q$、以及离散化方案。

[EDLM](../04-联合依赖与结构设计/4.3-因子化障碍与联合建模/4.3.2-edlm残差能量.md) 的重要性采样窗口 $w$ 决定哪些扩散时间做能量校正。本篇的 $\{t_k\}$ 决定连续时间切多碎。一个是提案上的重采样，一个是 CTMC 的离散化。都可以叫「少步」，账本不是同一页。窗口 $w=0.2$ 只校正 $t\in[0.8,1]$；几何网格 $\kappa\approx 1.3$ 让靠近终点的步更密。看起来都在「早段多花算力」，早段的定义相反：EDLM 的 $t$ 从 1 走到 0，早段是接近全掩；本篇反向时间从噪声走到数据，几何网格密的是靠近干净数据的一侧。两套时间箭头不要画在一张日程上。
...
- [采样与调度](../02-数学与生成机制/2.6-采样后验调度与重掩.md)
- [Score entropy](../02-数学与生成机制/2.5-score-entropy与离散流匹配/2.5.2-score-entropy.md)
- [ParallelBench](../07-控制、评测与选型/7.3-parallelbench与并行性评测.md)
- [离散 copula](../04-联合依赖与结构设计/4.3-因子化障碍与联合建模/4.3.1-离散copula.md)
- [EDLM](../04-联合依赖与结构设计/4.3-因子化障碍与联合建模/4.3.2-edlm残差能量.md)
- [提交之后还能不能改](./6.2-纠错重掩与动态调度.md)
- [少步蒸馏](../05-训练、后训练与迁移/5.3-少步与轨迹蒸馏/5.3.1-sdtt少步蒸馏.md)
- [从图像到离散](../02-数学与生成机制/2.1-从ddpm到离散token.md)
- [离散扩散](../02-数学与生成机制/2.2-离散扩散与转移矩阵.md)
- [失效模式](../07-控制、评测与选型/7.4-失效模式与复现清单.md)
