...
Credit 会和自信的错字共谋，规划器专文写过同一句。一直尖的错误中间量会提前落盘。hierarchical 用空间切开缓解局部依赖，不检查对错。验证器（[APD](./6.3-验证投机与流水线解码/6.3.1-apd与ssd.md)、SSD）仍要另计延迟。serving 把启发式跑快，没有把联合分布写回来。SDAR 在 H200、LMDeploy、8B、大 batch 上的 6600 TGS 是另一张卡，见[SDAR](../05-训练、后训练与迁移/5.2-sdar与块扩散转换.md)，不能减本篇 680。带 KV 时 credit 平均分掉到 51.56，说明「历史分」和过期缓存叠在一起可以比 DualCache 的 52.15 还差。消融表存在的理由就是挡住「四块全开一定最好」。
...
- [Qian et al., d3LLM, 2026](https://arxiv.org/abs/2601.07568) — 伪轨迹与 AUP；10.3× 对照 HF 上原版 LLaDA 27.9 TPS，专文见[d3LLM](../05-训练、后训练与迁移/5.3-少步与轨迹蒸馏/5.3.3-d3llm伪轨迹与aup.md)。不是本篇 Table 1。

## 相关

- [推理加速](./6.4-kv缓存与推理加速.md)
- [块扩散](../04-联合依赖与结构设计/4.2-块扩散与混合因式分解.md)
- [少步蒸馏](../05-训练、后训练与迁移/5.3-少步与轨迹蒸馏/5.3.1-sdtt少步蒸馏.md)
- [d3LLM](../05-训练、后训练与迁移/5.3-少步与轨迹蒸馏/5.3.3-d3llm伪轨迹与aup.md)
...
- [量化 dLLM](./6.6-量化与低精度部署.md)
- [Dream、Mercury、Seed](../03-模型谱系/3.3-dream-mercury-gemini与seed.md)
- [SDAR](../05-训练、后训练与迁移/5.2-sdar与块扩散转换.md)
- [APD](./6.3-验证投机与流水线解码/6.3.1-apd与ssd.md)
- [D2F](./6.3-验证投机与流水线解码/6.3.2-d2f脏前缀流水线.md)
- [ReFusion](./6.3-验证投机与流水线解码/6.3.3-refusion槽级规划.md)
- [SlowFast](./6.3-验证投机与流水线解码/6.3.4-slowfast动态采样.md)
