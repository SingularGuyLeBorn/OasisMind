...
1B 尺度上的配方扫描值得当成改编论文的读法，而不是轶事。作者先在 1B 上确认初始化、移位、噪声重标定各自有用，再把配方接到 7B。7B 上没有公开「去掉移位、其它全同」的完整对照表，所以 7B 的 Sudoku 81.0 不能单独归因于移位或单独归因于噪声重标定。能写的是：改编有配方，配方在小尺度上消融过，大尺度上作为整包交付。把 7B 表上的规划优势写成「因为他们用了 CART」，因果链中间缺一张 7B 消融。机制判断见[五条性质](../03-points/discreteness.md)。
