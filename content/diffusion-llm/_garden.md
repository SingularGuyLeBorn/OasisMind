...
| 03 知识点 | 块扩散、任意顺序、规划器、改编、缓存、Eso-LM、少步蒸馏、引导、对齐、代码向、离散流、失效 |
...
9. [从自回归改编](./03-points/ar-to-diffusion.md)  
   DiffuLLaMA / Dream / LLaDA 2.0 / Fast-dLLM v2。

10. [双向注意力与反转诅咒](./03-points/bidirectional-attention.md)  
    Berglund 的 0% 反向；LLaDA 诗句表正向 48.8 / 反向 42.4，GPT-4o 为 82.7 / 34.3。

11. [推理加速：近似缓存与并行揭开](./03-points/inference-acceleration.md)  
    Fast-dLLM DualCache、dKV-Cache、CAP。27.6× 的对照物是原版 LLaDA，不是 AR。

12. [Eso-LM：任意顺序损失，因果注意力换精确 KV](./03-points/eso-lm.md)  
    洗牌 + 原位置 RoPE。65× 对照无缓存 MDLM。不是 LLaDA 8B。

13. [少步蒸馏：把老师的 1024 步塞进学生的几十步](./03-points/few-step-distill.md)  
    SDTT；32 步约 4× 于带 KV 的 GPT-2。863M 质量，不要抄到 8B。

14. [可控生成与引导](./03-points/controllable-generation.md)  
    Diffusion-LM 连续梯度；离散 D-CFG；8B 实际在用的掩码与定长。

15. [对齐与强化学习](./03-points/alignment-rl.md)  
    VRPO / LLaDA 1.5；d1 / diffu-GRPO。原版 Instruct 没有 RL。

16. [代码向扩散：DiffuCoder、AR-ness 与 coupled-GRPO](./03-points/code-dllm.md)  
    7B 代码专料约 130B；Table 1–2；互补掩码估对数概率。不要和 Nie 的 35.4 横减。

17. [离散流匹配：概率路径比 Q_t 更宽的那一族](./03-points/discrete-flow.md)  
    DFM；吸收态 $1/t$ 是一条路径。1.7B HumanEval Pass@1 为 6.7%。

18. [失效模式](./03-points/failure-modes.md)  
    定长与 EOS、并行搭配、PPL 不可比、近似缓存过期。

🔴 **04 模型**

19. [代表性扩散语言模型一览](./03-models/representative-models.md)
20. [LLaDA：8B 从头训到 100B 改编](./03-models/llada-frontier.md)
21. [LLaDA-MoE：从头训的稀疏掩码扩散](./03-models/llada-moe.md)
22. [Dream、Mercury、Gemini Diffusion、Seed](./03-models/dream-mercury-seed.md)
23. [多模态扩散：LLaDA-V、MMaDA、Dimple](./03-models/multimodal-dllm.md)

⚖️ **05 对照**

24. [扩散 vs 自回归](./04-comparison/diffusion-vs-autoregressive.md)  
...
P(x) 怎么因式分解
        ...
        └─ 扩散：正向腐蚀 + 反向去噪
                ...
                └─ 离散：词表上的 Q_t
                        ...
                        │       ├─ 采样：置信度揭开 / remask（冻结规划器）；DDPD 另训脏净头
                        │       ├─ 变体：块扩散（块间 AR，块内扩散）
