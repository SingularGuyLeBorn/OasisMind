...
| 03 知识点 | 五条性质/CART/边际陷阱、CRoCoDiL、块扩散、任意顺序、规划器、提交后能否改、改编、SDAR、D2F、ReFusion、缓存、SlowFast、APD/SSD、copula、EDLM、CoDD、ParallelBench、serving、量化、Eso-LM、少步蒸馏、dParallel、伪轨迹/AUP、引导、嵌套 SMC、对齐、代码向、离散流、score entropy、自适应采样、失效 |
...
21. [EDLM：残差能量校正因子化去噪](./03-points/edlm.md)  
    给 MDLM 乘序列级能量。12 层 GPT-2 尺度，不是 8B。Table 3 GPT-2、2048 步：MDLM 34.9 对 EDLM-NCE 17.7，约 49%。1.3× 是单条 1024 token 约 13s 对 17s，$k=2$、$w=0.2$。OWT coAR 17.58 对 AR 17.56。不是 DCD 的 32× NFE。

22. [CoDD：在冻住的 dLLM 上接一层可算联合](./03-points/codd.md)  
    冻 LLaDA/Dream，HMM 回路 $N=1024$。+5.00 在低置信 MATH500 256 步。+10.84 在 Dream 熵 GSM8K 128 步。56.4 是 Dream 64 步，不是 LLaDA。约 3 GPU 小时。

23. [ParallelBench：GSM8K 测不出并行诅咒](./03-points/parallelbench.md)  
    一步 KL 下界是 $\mathcal{C}(Y|X)$。Shuffle 即使每步 2 token 也随 $n$ 趋向 0。微调修不好 $\mathcal{C}>0$。

24. [Serving：vLLM 的调度器接不上扩散](./03-points/serving.md)  
    dInfer 四块。8×H800、batch 1：680 TPS 对 Fast-dLLM 63、对 vLLM 上 Qwen2.5-3B 277。1100 是 TD 的 HumanEval 列。

25. [量化 dLLM：掩码态和去噪步都要管](./03-points/quantization.md)  
    STaR-Quant W4A4。LLaDA 均分 57.07 对 FP 58.99。1.69× 是 Dream 吞吐，3.14× 是 Dream 显存。另一个 1.69 是均分分差。A40。

26. [Eso-LM：任意顺序损失，因果注意力换精确 KV](./03-points/eso-lm.md)  
    洗牌 + 原位置 RoPE。65× 对照无缓存 MDLM。不是 LLaDA 8B。

27. [少步蒸馏：把老师的 1024 步塞进学生的几十步](./03-points/few-step-distill.md)  
    SDTT；32 步约 4× 于带 KV 的 GPT-2。863M 质量，不要抄到 8B。

28. [dParallel：把确定性逼成并行](./03-points/dparallel.md)  
    certainty-forcing。GSM8K-CoT 256 步 18.6s 到 30 步 2.2s，8.5×，分 75.7 到 76.1。MBPP 10.5×。RTX 6000 Ada。不是 Qian 的 TPF 5.14。

29. [d3LLM：伪轨迹蒸馏与 AUP](./03-points/d3llm.md)  
    顺序来自老师、字来自标准答。GSM8K-CoT 单卡 H100：288.9 TPS 对原版 27.9 约 10.3×。AUP 切掉掉分超过 5 点的工作点。

30. [可控生成与引导](./03-points/controllable-generation.md)  
    Diffusion-LM 连续梯度；离散 D-CFG；8B 实际在用的掩码与定长。

31. [嵌套 SMC：推理时把奖励拧进粒子](./03-points/nested-smc.md)  
    训练免费。MDLM 12 层 768、$T=50$。Table 1：$N=4,M=8,K=4,\lambda=10$，NSMC 毒性率 0.39 / PPL 42.3，FA-NSMC 0.40 / 42.9。不是 LLaDA 8B。毒性列是稀有事件探针。

32. [对齐与强化学习](./03-points/alignment-rl.md)  
    VRPO / LLaDA 1.5；d1 / diffu-GRPO。原版 Instruct 没有 RL。

33. [代码向扩散：DiffuCoder、AR-ness 与 coupled-GRPO](./03-points/code-dllm.md)  
    7B 代码专料约 130B；Table 1–2；互补掩码估对数概率。不要和 Nie 的 35.4 横减。

34. [离散流匹配：概率路径比 Q_t 更宽的那一族](./03-points/discrete-flow.md)  
    DFM；吸收态 $1/t$ 是一条路径。1.7B HumanEval Pass@1 为 6.7%。

35. [Score entropy：离散扩散在估比率](./03-points/score-entropy.md)  
    concrete score；$25\%{-}75\%$ 对照先前离散扩散。1BW 上界 $\leq 32.79$ 对 AR 31.98。不是 LLaDA 的损失。

36. [自适应采样：步数跟 DTC 走](./03-points/adaptive-sampling.md)  
    leave-one-out 一阶采样器。$N=\widetilde{O}(\mathrm{DTC}/\varepsilon)$，不是 $d$。$\tau$-leaping 的线性 $d$ 是采样器病。合成马尔可夫链与 $k$ 混合，$N=20/30/40$。不是 LLaDA 8B。DTC 不和 ParallelBench 的 $\mathcal{C}$、DCD 的 $\mathrm{D}_{TC}$ 焊。

37. [失效模式](./03-points/failure-modes.md)  
    定长与 EOS、并行搭配、PPL 不可比、近似缓存过期。

🔴 **04 模型**

38. [代表性扩散语言模型一览](./03-models/representative-models.md)
39. [LLaDA：8B 从头训到 100B 改编](./03-models/llada-frontier.md)
40. [LLaDA-MoE：从头训的稀疏掩码扩散](./03-models/llada-moe.md)
41. [Dream、Mercury、Gemini Diffusion、Seed](./03-models/dream-mercury-seed.md)
42. [多模态扩散：LLaDA-V、MMaDA、Dimple](./03-models/multimodal-dllm.md)

⚖️ **05 对照**

43. [扩散 vs 自回归](./04-comparison/diffusion-vs-autoregressive.md)  
...
P(x) 怎么因式分解
        ...
        └─ 扩散：正向腐蚀 + 反向去噪
                ...
                └─ 离散：词表上的 Q_t
                        │
                        ├─ 均匀跳转
                        │       └─ 自适应采样：leave-one-out；$N$ 跟 DTC 走；τ-leaping 的线性 $d$ 是采样器病；合成实验不是 8B
                        ├─ 吸收态 [MASK]  ← 2024 后主流
                        ...
                        │       ├─ EDLM：残差能量 × MDLM 提案；49% 是 Table 3 GPT-2 2048 步 34.9→17.7；1.3× 是 13s/17s；不是 8B
