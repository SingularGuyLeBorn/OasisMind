...
改编几何（移位、退火、WSD）见[从自回归改编](../05-训练、后训练与迁移/5.1-从自回归权重改编.md)。本篇只钉各家公开表和不能横比的吞吐。开源代码向检查点见[DiffuCoder](../05-训练、后训练与迁移/5.5-代码模型的训练与评测.md)，不要和下面四张交付物卡焊在一起。
...
Countdown 上调扩散步数，作者给出质量和速度的折中：步数设在 5–20 时，可以同时比 Qwen2.5 7B 更快更好。这是规划题上的曲线，不是 HumanEval 的默认。步数旋钮见[采样](../02-数学与生成机制/2.6-采样后验调度与重掩.md)。
...
1B 尺度上的配方扫描值得当成改编论文的读法，而不是轶事。作者先在 1B 上确认初始化、移位、噪声重标定各自有用，再把配方接到 7B。7B 上没有公开「去掉移位、其它全同」的完整对照表，所以 7B 的 Sudoku 81.0 不能单独归因于移位或单独归因于噪声重标定。能写的是：改编有配方，配方在小尺度上消融过，大尺度上作为整包交付。把 7B 表上的规划优势写成「因为他们用了 CART」，因果链中间缺一张 7B 消融。机制判断见[五条性质](../04-联合依赖与结构设计/4.3-因子化障碍与联合建模/4.3-因子化障碍与联合建模.md)。
