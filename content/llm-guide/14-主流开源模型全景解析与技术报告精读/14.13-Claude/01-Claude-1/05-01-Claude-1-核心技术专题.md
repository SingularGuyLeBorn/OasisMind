...
**2026-08 勘误**：上面这棵「推测架构」树 **不是** Anthropic 对 Claude 1 的官方规格。CAI 论文（[arXiv:2212.08073](https://arxiv.org/abs/2212.08073)）实验曲线里的 **52B** 是研究用语言模型尺寸（Figure 2–4），不能写成产品参数。层数、隐藏维、GQA、RoPE、SwiGLU 在 2023-03-14 博文和论文里都没有对应到 Claude API。9K 窗口的一手是 [100K 扩窗公告](https://www.anthropic.com/news/100k-context-windows)（from 9K to 100K），不是逆向延迟估算。数字与拆面以 [D2 公开材料精读](./01-01-Claude-1-架构精译.md) 为准。不删上面 2025 推测正文。
