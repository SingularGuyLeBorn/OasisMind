...
阶跃星辰（StepFun）的 `Step` 名称同时覆盖云端产品、研究模型、开放权重检查点和图像/视频/语音生成支线。这些身份不能合并成一条“都已开源、都能本地部署”的谱系。
...
2. **Step-3 才有可复核的模型—系统协同报告**：MFA 注意力降低 KV 与计算开销，AFD 把 Attention 与 FFN 分开部署；模型是 321B 多模态 MoE，不能归类为纯语言模型。
