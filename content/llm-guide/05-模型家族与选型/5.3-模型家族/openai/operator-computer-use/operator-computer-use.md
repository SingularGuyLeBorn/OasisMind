...
旧稿曾把浏览器自动化实现细节和精确内部训练阶段写成确定事实；公开材料不足以支持这些内部结论。Operator/CUA 系统卡确实报告了 OSWorld、WebArena、WebVoyager 等基准，但这些分数只在对应版本、日期和评测协议内成立，不能反推出未披露训练实现，也不能直接代表真实生产安全性。
