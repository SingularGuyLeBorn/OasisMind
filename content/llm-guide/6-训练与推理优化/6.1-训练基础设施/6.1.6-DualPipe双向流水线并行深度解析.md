...
> 图 1：上轨正向 micro-batch、下轨反向。中间一对块示意 Attention 计算与 All-to-All dispatch、MLP 计算与 All-to-All combine 重叠。Rank 0 / Rank 7 只标明双向端点，不是把两张卡画成同一条时间轴的两行。无时间刻度。气泡数字用上面 §4 的表，不另画假 Gantt。
