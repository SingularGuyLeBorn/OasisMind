...
- **汉字 ≥ 4000**（去掉 YAML 后 `[\u4e00-\u9fff]`）。不够 = 不合格，重写。禁止注水凑字。材料不够必须文首声明 + `[OM-FREEPLAY]`。
- **落点：** 机制文不得写成硬件综述。MoE 路由/共享专家/STE/LatentMoE 在 `2.4.1`；EP/All-to-All/Grouped GEMM 在 `6.1`；量化在 `6.3`；加速器板级在 `9.1`。夹错章的综述不要当机制主线加厚。
