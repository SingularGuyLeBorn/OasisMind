...
- 状态：用户 Goal 续跑：成体系补专文 + 配图 + 人味。Loop Transformer 已入库。4.4：TRPO / RAFT / 薄节 / GxPO 已入库；首页 DAPO/GSPO 已改正。正在重写 DPO / ORPO。
- 当前波次：**DPO 去讲义腔配图** + **ORPO 去讲义腔配图**。wave-2 三张仍在跑；`fig-moe-router-top2` 不重画。不要抢 `4.6.2`。
