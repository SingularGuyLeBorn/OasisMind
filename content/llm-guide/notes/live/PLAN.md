...
1. **立刻做：** 回收 BOND（4.4.4/09，2407.14622）。交卷后链进 4.4.4。
2. **并行：** 继续补 4.4；同层满 10 先拆小节。不要发 4.4.1/11。4.4.4 到 09 后不要发 11。
...
- 上一件：入库 Online IPO（4.4.4/08），链进地图。
- 现在立刻做：回收 BOND。不要 `move_agent_to_root`。不要 push。
