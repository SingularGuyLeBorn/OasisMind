...
1. **立刻做：** 结构 **S1** `git mv`：`4.1-后训练概述`→`4.0`；ScaleRL→`4.8`；CLIP→`8.8`。改链接，不删旧文件。不要和未提交的别的章抢。第 5 章禁止再新建根级 `01-型号`。不要把 `01`–`10` 改成 `4.6.1`–`4.6.10`。
2. **结构余量：** S2–S7（5/14 分工已声明；综述改导航：`2.3.4` / `6.4.2` / `7.3`）。Connest5 仍留条。
3. **0.8 持续：** OPD 波 **01–04 / 09 / 10** 已入库。**G-OPD / SCOPE** 未核一手不升格。V4 后训练不 mkdir。清单勾完继续补知识点。
...
- 上一件：0.8 已勘误 **03-SDFT**（Table 5 单任务分母；Reverse/Forward 分叉；EMA）。
- 现在立刻做：结构 **S1** `git mv`。不要 `move_agent_to_root`。不要 push。
