...
1. **立刻做：** 4.4.4 已满到 10，先拆小节，不要发 11。不要发 4.4.1/11。
2. **并行：** 不要抢 `4.6.2`（opd-survey）。wave-2 四张仍 running。
3. **再下一波：** `fig-moe-router-top2` **不要重画**。不要发 4.4.1/11。

## 续跑锚点（不是停机指令）

刚做完的上一件，以及现在该立刻做的下一件。**不要把本节理解成「本回合可以收工」。**

- 上一件：入库 WARP（4.4.4/10），链进地图。
- 现在立刻做：4.4.4 拆小节，不要发 11。不要 `move_agent_to_root`。不要 push。
...
| jrl-446 | done | `4.4.6-其他策略梯度/02-JustRL-极简配方/` + 4.4.6 节首页加 02 行 | 已入库。4011 汉字；两图；九项平均 54.87%/64.32% 不是 AIME 单列；clip $[0.8,1.28]$ |
...
| oaif-442 | done | `4.4.2-无奖励模型的对齐DPO-KTO/06-OAIF-在线AI反馈/` | 已入库。4006 汉字；两图；Table 2/3；4-way 58.00%；不是 Lee RLAIF、不是 SPIN |
| oipo-444 | done | `4.4.4-其他对齐技术/08-Online-IPO-在线偏好/` | 已入库。4020 汉字；两图；Prop 4.1/4.2/5.1；Table 2；不是 OAIF 64.81% |
...
| warp-444 | done | `4.4.4-其他对齐技术/10-WARP-权重平均策略/` | 已入库。4298 汉字；两图；三次平均（EMA $\mu=0.01$、任务向量 SLERP、LITI $\eta=0.3$）；Table 1 Mixtral 第三轮 0.18 后停滞；GSM8K $55.6\to 66.8$；不是 WARM、不是 J-BOND |
