...
1. **立刻做：** 结构 **S1 余量**：`4.4.1` 夹与同号散文件；`Muon/` 在地图上声明或改 `6.5.2`；路径含 `：` 的再 `git mv`。第 5 章禁止再新建根级 `01-型号`。不要把 `01`–`10` 改成 `4.6.1`–`4.6.10`。
2. **结构余量：** S2–S7（综述改导航：`2.3.4` / `6.4.2` / `7.3`）。Connest5 仍留条。
...
- [x] **结构 S1a**：概述→`4.0`；ScaleRL→`4.8`；CLIP→`8.8`。同级 C 不再撞这三处。
- [ ] **结构 S1 余量 / S2–S7**：`4.4.1` 同号、Muon、`：`；综述改导航。不删文件。不要把 `01`–`10` 改成 `4.6.1`–`4.6.10`。

## 续跑锚点（不是停机指令）

刚做完的上一件，以及现在该立刻做的下一件。**不要把本节理解成「本回合可以收工」。**

- 上一件：S1a 已 `git mv` 概述→4.0、ScaleRL→4.8、CLIP→8.8。
- 现在立刻做：S1 余量（`4.4.1` 同号 / Muon / 冒号）。不要 `move_agent_to_root`。不要 push。
