...
1. **立刻做：** 勘误 **03-SDFT**（arXiv:2601.19897）。OPD 波 **01 / 02 / 04 / 09 / 10** 已入库。**G-OPD / SCOPE** 未核一手不升格。V4 后训练不 mkdir。子代理禁止改 `4.6-OPD.md`。生图必须浅色主题。
2. **结构：** S0b 地图已交（第 4、13 章首页按磁盘；4.6 一行改成 On-Policy Distillation）。下一步 S1 才 `git mv`：概述→`4.0`，ScaleRL→`4.8`，CLIP→`8.8`；不要和 4.6 专文抢同一文件。第 5 章禁止再新建根级 `01-型号`。不要把 `01`–`10` 改成 `4.6.1`–`4.6.10`。
...
- [ ] **结构 S1–S7**（S0 / S0b 地图已交）：见 `notes/chapter-structure-plan.md`。S1 拍板：概述→4.0、ScaleRL→4.8、CLIP→8.8。不删文件。不要把 `01`–`10` 改成 `4.6.1`–`4.6.10`。

## 续跑锚点（不是停机指令）

刚做完的上一件，以及现在该立刻做的下一件。**不要把本节理解成「本回合可以收工」。**

- 上一件：结构方案落地为 C/A/D + S0b 地图（第 4、13 章）。03-SDFT 仍在租约。
- 现在立刻做：等 **03-SDFT** 交卷后质检入库。不要 `move_agent_to_root`。不要 push。S1 `git mv` 另开切片。
