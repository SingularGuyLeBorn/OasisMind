...
1. **立刻做：** 结构 **S3**：`2.3.4` 收成导航（FA/H2O 推导改链到专文，原文不删）。可派子代理独租该文件。
2. **结构余量：** S4–S8（`6.4.2` / `7.3` / 过满拆篮：4.6 的 `01`–`10` 进 `4.6.1`，2.3.2 的 11–17 拆节）。Connest5 仍留条。新专文：**点分号最多三层；`01` 只挂在 `N.N.N` 下；同层约 ≤10。**
...
- [ ] **结构 S3–S8**：`2.3.4` 导航；过满拆篮（4.6 / 2.3.2）。新路径：三层点分号 + 第四层 `01`–`10`。不删文件。

## 续跑锚点（不是停机指令）

刚做完的上一件，以及现在该立刻做的下一件。**不要把本节理解成「本回合可以收工」。**

- 上一件：编号口径写入规划（三层 + 每层≤10 + `01` 只在第四层）；4.6 / 2.3.2 地图已写「不要加 11/18」。
- 现在立刻做：结构 **S3**（`2.3.4` 收成导航）。不要 `move_agent_to_root`。不要 push。
