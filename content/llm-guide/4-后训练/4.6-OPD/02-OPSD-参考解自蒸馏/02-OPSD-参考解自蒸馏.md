...
读法:「AIME25 从 37.1 到 43.4」应改成 **三集平均 37.1→43.4**;AIME25 单列是 **36.7→43.9**(仍是 1.7B,Avg@12,100 步内最好).8B 的 HMMT25 上 GRPO 46.7 略高于 OPSD 45.8,不要写成「三集全面碾压 GRPO」.SFT 三集都掉,论文归因于参考解写得短,测时生成变短;OPSD 把短解当成特权信息做合理化,而不是照抄.
...
Forward KL 的 41.1 和 Reverse KL 的 35.0 是 Table 3 **第 100 步**,不是峰值.Table 3 分母:AIME25,Qwen3-1.7B,Avg@12,同一套 pointwise clipping.
...
Algorithm 1:抽 minibatch → 每题学生采样一条 $\hat y$ → 按 (R3) 算 $\ell$ → batch 平均后更新 $\theta$.「只需 1 条探索轨迹」对应 Table 6 的 Number of Generations = 1,不是「整个训练只 roll 一次」.
...
$D$ 可以是 Forward KL,Reverse KL 或 $\operatorname{JSD}_\beta$(式 (7)).表上 Forward KL 赢,见上节 Table 3.**在这份特权教师比较尖的设置里,教师加权的 Forward KL 更好**;不要写成 OPD 家族定理.勾股定理那组 0.8 / 0.1 是示意图,论文表里没有,当数值例即可.
...
style 列比 math 列高一个数量级.不加处理,梯度会去学「therefore / okay」,不学「14」.论文在**词表项**上做 pointwise clip,不是对已经求和的 KL 做 `clamp(0, 10)`.对 $f$-散度的逐项贡献
...
即使有 $y^{\star}$,Appendix A 写得很死:题目难过模型的理解阈值,教师「开卷」也讲不明白,监督是噪声.他们只做到 **8B**,再大未测.「底座太蠢则开卷也看不懂」方向对,来源是这篇附录,不是发挥.

其它边界:

| 现象 | 原因 | 说明 |
| --- | --- | --- |
| 捷径 / 跳步 | $y^{\star}$ 过短时教师过度自信 | 论文自己更强调 SFT 会被短解压短思维,OPSD 用合理化把短解变成逐步软标签,**不是**保证不跳步 |
