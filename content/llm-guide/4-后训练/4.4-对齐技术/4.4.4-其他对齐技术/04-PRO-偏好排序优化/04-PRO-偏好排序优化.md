...
分子是整段回答的逐步对数概率。分母是长度。不除长度，长回答的对数和更负，排序会系统惩罚长回复。这一点和 [RRHF](../02-RRHF-排序响应对齐/02-RRHF-排序响应对齐.md) 的 $p_i$ 同构。SFT 那一项并不做这次除法，不要把两个归一焊成一个开关。
...
奖励差越大，温度越低，正例对这条负例的对比越尖。$\mathcal{T}^{k}_{k}$ 取所有负例温度的最小，用来对齐分子分母。消融里：单加温度，整体略涨；拿掉 $\mathcal{L}_{\mathrm{SFT}}$ 再拿掉温度，分数会塌。论文判断温度的作用接近「别把分差很小的负例一棍子打死」，$\mathcal{L}_{\mathrm{SFT}}$ 抬第一名的权重，两件事有重叠。ChatGPT 扩到序长 3 时，关掉温度总分反而到 68.40，略高于完整 PRO 的 67.97。温度不是无条件加分。高质量长序上它可能过尖。
...
也不是 RAFT / BoN。BoN 只对 RM 最高的那条做 SFT，$K-1$ 条丢掉。PRO 的负例进 softmax。加长序之后 BoN 会变强，Table 1 里 ChatGPT-3 的 BoN 总奖励 63.83，仍低于 PRO 的 67.97。CoH 靠提示词把好坏回答并排塞进上下文，走语义理解，BLEU 有时更高，奖励在 raw 上只有 45.00。
...
相对 SFT 高 6.52，相对 DPO 高 2.60。Harmless$_{\mathrm{base}}$ 上 PRO 奖励 62.96，对 DPO 的 54.43 拉开一截；Helpful$_{\mathrm{base}}$ 上 DPO 是 50.13，PRO 是 48.51，PRO 并不是四个子集都赢。论文把 harmless 更容易归因到礼貌、拒绝这类表层特征；helpful 要具体建议，7B 的世界知识不够，listwise 也补不上。
...
Table 3 消融。raw 上去掉 $\mathcal{L}_{\mathrm{SFT}}$：BLEU 从 21.54 掉到 9.85，奖励 55.35→53.25。去掉温度：21.41 / 55.04，几乎不动。两项一起去掉：5.14 / 46.17。Alpaca-3 上去掉 $\mathcal{L}_{\mathrm{SFT}}$ 奖励反而到 59.71（完整 PRO 是 58.72），BLEU 从 22.11 掉到 18.29，奖励模型被讨好、句子变差。只留式 (4) 的第一项、关掉 $k>1$：BLEU 21.10、奖励 58.11。ChatGPT-3 上关掉 $k>1$：22.80 / 67.75；去掉 $\mathcal{L}_{\mathrm{SFT}}$：21.84 / 67.84；关掉温度：22.98 / 68.40。递归那 $n-2$ 项不是装饰，但高质量长序上第一项已经很响，增量变小。两项一起去掉，ChatGPT-3 也塌到 6.25 / 43.16。$\mathcal{L}_{\mathrm{SFT}}$ 和温度至少留一个，模型才知道「分差很小的负例不要往死里罚」。
...
$n=1$ 时式 (4) 没有项，退回 SFT。没有序、也没有 RM 可排，式 (4)(5) 没有输入。要逐步过程奖励、要在线 PPO 那套探索，这篇的离线 listwise 帮不上。附录 A 把可微这件事写死：对齐目标和 SFT 可以单阶段相加；PPO 的奖励是离散的，只能先 SFT 再 KL 约束，两截训练。附录 E 给过一个点火偷车的多轮例子：上下文已经把助手带跑，对齐过的 7B 仍可能顺着写操作步骤。listwise 监督的是整段回答的序，不是每一轮的拒绝。更细的 turn-level 监督这篇没有做。Ethics 声明数据里有攻击性内容，只作研究，不代表作者立场。把有害偏好排成序，算法一样能拟合，不该拿去用。资助写在文末：国家重点研发计划 2022ZD0116308，国家自然科学基金 62036001。作者单位是北京大学与阿里巴巴。
