...
published: true
excerpt: "从威胁模型、最小权限、沙箱、审批和评估理解高权限 Agent 的安全边界。"
---
...
> 图 1：把「护栏」画成可检查的层，而不是科幻光罩。外层是策略与审批，内层是工具权限。来源：本知识库自绘概念图，综合本文威胁模型，不对应单一论文原图。
...
> 图 2：间接注入的路径是「工具读到的数据里藏指令」，不是用户在聊天框里明说。防御要落在工具输出进模型之前的隔离，而不是只靠系统提示里写「你要小心」。来源：本知识库自绘概念图，综合本文攻击路径，不对应单一论文原图。
...
Agent 生成的任意代码不应直接继承宿主机的完整权限。代码解释器和不受信任二进制应进入与风险匹配的隔离环境；普通 API 工具则至少需要服务端鉴权、参数校验、最小权限、网络边界和高风险操作审批。不是所有只读工具都必须放进同一种“强沙箱”，隔离等级应由威胁模型和数据敏感度决定。
...
安全审计不要求记录或展示模型的隐藏思维链。应记录足以重放责任链的可观察事件，使审计人员能够回答：
1. Agent 接收到了什么输入？
2. 系统保存了什么计划摘要、策略判定和授权结果？
3. 模型提出了什么动作与参数，宿主为何批准或拒绝？
4. 工具的返回结果是什么？

结构化日志至少包含经过必要脱敏的输入/输出摘要、动作提议、参数、调用者身份、策略与审批结果、工具返回、验证结论和时间戳。对密钥、个人数据与敏感正文执行最小化和分级留存，并使用访问控制、完整性校验和保留期限；不要把完整隐藏推理当作审计前提。
