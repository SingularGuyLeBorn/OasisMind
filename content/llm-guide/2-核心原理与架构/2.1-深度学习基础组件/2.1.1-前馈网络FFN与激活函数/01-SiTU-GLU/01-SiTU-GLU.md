---
title: "01 · SiTU-GLU：用光滑上界控制 SwiGLU 的异常激活"
date: 2026-08-30
as_of: 2026-08-30
tags: [SiTU-GLU, SwiGLU, 激活函数, FFN, Kimi-K3, LatentMoE]
---

# 01 SiTU-GLU：用光滑上界控制 SwiGLU 的异常激活

SwiGLU 用两条支路的逐元素乘积构造 FFN 的中间激活。当门支路和上投影（up）支路同时出现大值时，乘积会迅速放大，并形成 activation outlier。模型采用低精度训练或推理时，这类异常值更容易造成溢出和量化误差。

Kimi K3 在专家 FFN 中采用 SiTU-GLU（Sigmoid Tanh Unit GLU）处理这个问题。它保留 SwiGLU 在原点附近的形状，同时用 $\beta\tanh(x/\beta)$ 分别限制两条支路的幅度。K3 取 $\beta_1=4$、$\beta_2=25$，因此两条支路相乘后，每个坐标的绝对值都小于 $100$。

这句话只描述 **SiTU-GLU 乘积本身**。后续线性层仍然可以放大输出，所以 $100$ 不是整个 FFN、MoE 层或残差流的全局上界。

> 相关内容：[FFN 与激活函数](../2.1.1-前馈网络FFN与激活函数.md) · [GLU 家族](../03-GLU家族-从GLU到SwiGLU/03-GLU家族-从GLU到SwiGLU.md) · [PowLU](../04-PowLU-Ling对SwiGLU的稳定化改写/04-PowLU-Ling对SwiGLU的稳定化改写.md) · [Stable LatentMoE](../../../2.4-前沿架构与变体/2.4.1-混合专家模型MoE/10-Stable-LatentMoE与Quantile-Balancing/10-Stable-LatentMoE与Quantile-Balancing.md) · [Kimi K3](../../../../05-模型家族与选型/5.3-模型家族/kimi/kimi-k3/kimi-k3.md)

---

## 1. SwiGLU 为什么会产生大激活

先统一记号。本文采用列向量约定。设专家接收的输入为 $\bm{z}\in\mathbb{R}^{\ell}$，两条支路的预激活分别是
...
=
\bigl[\bm{g}\odot\sigma(\bm{g})\bigr]\odot\bm{u}.
\tag{1}
$$

其中 $\odot$ 表示逐元素乘法。问题出在两个无界因子：

- 当 $g_i$ 很大且为正时，$\sigma(g_i)\approx 1$，门支路近似 $g_i$；
- up 支路的 $u_i$ 本身也是无界的；
- 如果同一坐标上的 $g_i$ 和 $u_i$ 同时很大，输出坐标就近似 $g_i u_i$。

例如，为了观察增长速度，暂时令某个坐标上的 $g_i=u_i=100$。此时 SwiGLU 的输出约为 $100\times100=10\,000$。真实模型中的 $\bm{g}$ 和 $\bm{u}$ 来自不同矩阵，不会处处相等；这个简化例子只说明两个大值相乘时，动态范围会迅速扩大。

K3 的路由专家位于一条较长的计算路径中：token 先从隐藏维 $d$ 降到潜在维 $\ell$，经过专家 FFN，再升回 $d$。在 2.78T 参数规模和混合精度环境下，专家内部的异常激活会增加溢出风险。后面的 RMSNorm 可以调整聚合结果的尺度，却无法挽回已经在专家内部溢出的数值。

---

## 2. SiTU-GLU 改了什么

SiTU-GLU 先定义一个平滑的幅度限制函数：
...
当 $|x|$ 较小时，softcap 接近恒等函数；当 $|x|$ 增大时，输出逐渐趋近于 $\pm\beta$。它没有硬截断产生的折点。

SiTU-GLU 将 SwiGLU 的两条线性因子都替换为 softcap：
...
> 图 1：SiTU-GLU 分别限制门支路和 up 支路的幅度。图中使用同一个标量横轴比较函数形状；真实模型中的两条预激活分别是 $\mathbf{W}_g\bm{z}$ 和 $\mathbf{W}_u\bm{z}$。
...
门支路每个坐标的绝对值小于 $\beta_1$，up 支路每个坐标的绝对值小于 $\beta_2$。因此
...
这个界只作用于专家中间维上的逐元素乘积。输出投影 $\mathbf{W}_2$、专家路由加权和以及升维矩阵 $\mathbf{W}^{\uparrow}$ 都可能再次放大坐标。
...
因此它在 $x=0$ 处的函数值为 $0$，一阶导数为 $1$，与恒等函数的差异从三次项开始。输入不大时，SiTU-GLU 的两条支路仍然近似 SwiGLU；只有输入继续增大时，softcap 才逐渐限制幅度。
...
当 $\beta_1,\beta_2\to\infty$ 时，两个 softcap 都逐点趋近于恒等函数，SiTU-GLU 也随之趋近于 SwiGLU。
...
这并不意味着 softcap 在深度饱和区仍能提供很强的梯度。更准确的说法是：它没有硬截断的折点，梯度会连续衰减，而不是越过阈值后立即变为零。K3 报告给出了这一设计动机，但没有提供 SiTU-GLU 与硬截断之间的独立消融表。

---

## 4. SiTU-GLU 在 K3 LatentMoE 中的位置

SiTU-GLU 解决的是 **专家内部的激活动态范围**，不是 K3 路由系统的全部稳定性问题。
...
这里，$d=7168$、$\ell=3584$、$N_s=2$，$\mathcal{T}_k$ 表示从 896 个路由专家中选出的 Top-16。路由器根据完整的 $\bm{x}$ 选择专家；只有发送给路由专家的表示会先降到 $\ell$。这个 $\ell$ 是 LatentMoE 的专家通信维度，与 MLA 的 $c^{KV}$ 无关，也不会进入 KV cache。
...
| V4 SwiGLU clamp | 对线性支路和门支路做硬截断 | 有界，但存在折点 | FFN 激活 |
...
K3 报告给出了 SiTU-GLU 的公式、固定超参数和设计动机，但没有提供“只替换激活函数”的独立消融实验。阅读报告时需要区分以下结论：

- $\beta_1=4$、$\beta_2=25$ 和坐标上界 $100$ 是 SiTU-GLU 的明确配置；
- K2 使用 SwiGLU，K3 的模型配置表改为 SiTU-GLU；
- 报告对软限制优于硬截断的描述是定性结论，没有独立百分比；
- 验证损失和下游表现的改善出现在 RMSNorm 的讨论中，不能直接算作 SiTU-GLU 的收益；
- Fig. 7 中约 $2.5\times$ 的 scaling efficiency 来自 K3 的整体架构、数据和训练方案，不是 SiTU-GLU 的单项加速比；
- K3 后训练使用 MXFP4 权重和 MXFP8 激活，但报告没有给出 SiTU-GLU 与量化训练组合的独立消融；
- Shazeer 的 T5-base 实验说明 SwiGLU 相对普通 FFN 的效果，不是 K3 或 SiTU-GLU 的实验结果。

因此，现有证据支持的结论是：SiTU-GLU 为 K3 提供了一个明确、平滑且坐标有界的专家激活方案。它是否单独改善最终模型指标，报告没有用独立消融回答。
...
继续阅读：[GLU 家族](../03-GLU家族-从GLU到SwiGLU/03-GLU家族-从GLU到SwiGLU.md) · [PowLU](../04-PowLU-Ling对SwiGLU的稳定化改写/04-PowLU-Ling对SwiGLU的稳定化改写.md) · [Stable LatentMoE](../../../2.4-前沿架构与变体/2.4.1-混合专家模型MoE/10-Stable-LatentMoE与Quantile-Balancing/10-Stable-LatentMoE与Quantile-Balancing.md) · [Kimi K3](../../../../05-模型家族与选型/5.3-模型家族/kimi/kimi-k3/kimi-k3.md)
