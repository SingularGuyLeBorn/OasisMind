...
ReGLU、GEGLU 与 SwiGLU 保留相同的“门乘值”结构，差别在门支路采用的激活函数。常见的 $8d/3$ 中间宽度来自两矩阵 $4d$ FFN 的参数量对齐，并非所有模型必须采用的固定比例。单路 ReLU、GELU 与 SiLU 的曲线见 [02 激活函数谱系](../02-激活函数谱系-从饱和到软门/02-激活函数谱系-从饱和到软门.md)；低精度截断、光滑上界与幂次改写分别见 [01 SiTU-GLU](../01-SiTU-GLU/01-SiTU-GLU.md)、[04 PowLU](../04-PowLU-Ling对SwiGLU的稳定化改写/04-PowLU-Ling对SwiGLU的稳定化改写.md) 和 [6.1.7 训练稳定性](../../../../6-训练与推理优化/6.1-训练基础设施/6.1.7-训练稳定性与训推不一致.md)。
...
其中 $W_{1}\in\mathbb{R}^{d\times d_{ff}}$、$W_{2}\in\mathbb{R}^{d_{ff}\times d}$。若去掉中间非线性，两层线性变换可以合并为一次线性映射，该 FFN 也就不再具有非线性表达能力。后文以式 (2) 作为门控结构的比较基线。

T5 代码库去掉了偏置，Shazeer 的实验也采用无偏置设置，因此以下 FFN 公式省略 $b$、$c$：
...
$\sigma$ 是 sigmoid，$\otimes$ 是 Hadamard 积（与 $\odot$ 同义）。Bilinear 是不使用激活函数的双线性变体，两条线性投影直接逐元素相乘。Shazeer 将其追溯到 Mnih & Hinton（2007）的双线性层。
...
形状：$W,V\in\mathbb{R}^{d\times d_{ff}'}$，$W_{2}\in\mathbb{R}^{d_{ff}'\times d}$。式 (5) 相对式 (2) 多了一套与 $W$ 同形状的 $V$。实现上常把 $W$ 与 $V$ 拼成一次 `gate_up_proj`：$d\to 2d_{ff}'$，再沿隐藏维拆分为门分支和值分支；这与两次独立 `Linear` 在代数上等价。
...
实验设定与 T5-base 相同：C4 上做 span-filling，Adafactor，预训练 **524,288** step；短训 65,536 step 用于估计不同运行之间的方差。主指标是 held-out shard 上训练目标的 **log-perplexity**（越低越好）。所有行按上一节对齐参数与计算量。下表列出 Shazeer Table 1 中 524,288 step 的结果：
...
2. **门控结构带来更明显的差异。** GLU 为 1.663，Bilinear 为 1.648，ReGLU 为 1.645；GEGLU 为 **1.633**，SwiGLU 为 **1.636**。在这一 T5 设定中，二者属于论文概括的最优组；论文没有进一步研究跨设定排序或作用机制。

下游 GLUE、SuperGLUE 与 SQuAD 的 Table 2–4 波动更大，Shazeer 只总结为新变体在多数任务上更好。因此，结构间的直接比较以 Table 1 为准。

---

## 6. 后续模型中的 SwiGLU

Shazeer 2020 的实验表明：在 T5 设定中，GLU 变体的 perplexity 更低，GEGLU 与 SwiGLU 表现接近，作用机制仍待解释。Llama、Qwen 与 DeepSeek 的选型来自之后各自的模型设计。

后续模型逐渐将式 (5) 的 $\mathrm{FFN}_{\mathrm{SwiGLU}}$ 作为默认 FFN：

- **Llama 1**（Touvron et al., 2023）：用 Shazeer 的 SwiGLU 替换 ReLU，中间宽取 $\tfrac{2}{3}\times 4d$；Llama 2 的架构声明沿用这一设计。
...
门函数改写与低精度截断见 [04 PowLU](../04-PowLU-Ling对SwiGLU的稳定化改写/04-PowLU-Ling对SwiGLU的稳定化改写.md) 和 [6.1.7 训练稳定性](../../../../6-训练与推理优化/6.1-训练基础设施/6.1.7-训练稳定性与训推不一致.md)；各模型如何组合这些组件见 [第 05 章](../../../../05-模型家族与选型/05-模型家族与选型.md)。
...
| 对象 | 研究问题 | 详见 |
|------|----------|------|
| 单路 ReLU / GELU / SiLU | 两矩阵 FFN 中点式激活函数的性质 | [02 激活函数谱系](../02-激活函数谱系-从饱和到软门/02-激活函数谱系-从饱和到软门.md) |
| SiTU-GLU | 为门支路和值支路加入光滑上界 | [01 SiTU-GLU](../01-SiTU-GLU/01-SiTU-GLU.md) |
| PowLU | 保持正半轴渐近线性的门函数改写 | [04 PowLU](../04-PowLU-Ling对SwiGLU的稳定化改写/04-PowLU-Ling对SwiGLU的稳定化改写.md) |
| SwiGLU clamp | 训练稳定性中的硬截断 | [6.1.7 训练稳定性](../../../../6-训练与推理优化/6.1-训练基础设施/6.1.7-训练稳定性与训推不一致.md) |

完整的小节地图见 [2.1.1 前馈网络与激活函数](../2.1.1-前馈网络FFN与激活函数.md)。
