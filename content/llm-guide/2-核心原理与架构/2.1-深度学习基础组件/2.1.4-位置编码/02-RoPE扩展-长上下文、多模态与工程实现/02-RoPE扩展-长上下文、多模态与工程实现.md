...
这篇接 [01 RoPE 本体](../01-RoPE本体-旋转位置编码/01-RoPE本体-旋转位置编码.md)：本体已经证明相对距离会出现在点积里。这里只收它落到真实长度、真实坐标轴、真实内核之后的张力。节地图仍在 [2.1.4](../2.1.4-位置编码.md)。不要在这里重推式 (8)。
...
![高频相位绕圈、NTK 调 base、YaRN 再加热、PI 压索引](./images/fig-rope-ntk-yarn.png)

> 图 1：长上下文补丁对照。不是论文损失曲线，不要把图里的圆弧当实测相位。公式以本节式 (1)–(4) 为准。

**图 1 解析**

- 左：同一高频平面从 4K 拉到 128K，转角跨过多个 $2\pi$，训练时没见过这些相位。
- NTK-aware：抬高 base，同样长度转得慢；远程稳了，短程分辨率会一起被削。
- YaRN：频率缩放之外再除温度 $t$，把 softmax 锐度拉回训练附近。
- 下排 PI 是「把长地图压回短坐标系」；渐进式训练是真的见过长分布，最贵。
