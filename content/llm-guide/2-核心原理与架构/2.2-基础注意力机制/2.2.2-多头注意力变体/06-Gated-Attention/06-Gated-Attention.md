...
其中 $X$ 是 **pre-norm 之后** 的隐状态(论文式 (5) 脚注 1),不是残差主干上未经归一化的 $h$.记号沿用 [01-MHA](../01-MHA-多头注意力的标准形式/01-MHA-多头注意力的标准形式.md) 的 $Q,K,V,W_O$;本篇只回答「门加在注意力子层的哪一截,为什么 $G_1$ 赢」.它**不是** [03-Gated Residual](../../../2.1-深度学习基础组件/2.1.3-残差连接/03-Gated-Residual/03-Gated-Residual.md) 的四分支残差读门,也**不是** [SwiGLU](../../../2.1-深度学习基础组件/2.1.1-激活函数/02-GLU家族-从GLU到SwiGLU/02-GLU家族-从GLU到SwiGLU.md) / [PowLU](../../../2.1-深度学习基础组件/2.1.1-激活函数/03-PowLU-Ling对SwiGLU的稳定化改写/03-PowLU-Ling对SwiGLU的稳定化改写.md) / [SiTU](../../../2.1-深度学习基础组件/2.1.1-激活函数/01-SiTU-GLU/01-SiTU-GLU.md) 那种 FFN 激活.Qwen3-Next 把推荐的 SDPA 输出门捆进产品,版本事实只链 [Qwen 模型家族](../../../../05-模型家族与选型/5.3-模型家族/qwen/qwen.md) / [Qwen3.5](../../../../05-模型家族与选型/5.3-模型家族/qwen/qwen3-5/qwen3-5.md),不在这里抄整份 Next 报告.
...
![Q,K,V 进 SDPA,之后是推荐的逐头 sigmoid 门 G1,再 Concat 和 W_O;右侧虚线标 G2–G5 不是推荐位置](./images/redrawn-fig-gated-attn-g1-after-sdpa.png)
...
![左:注意力子层里 SDPA 输出门;右:四条残差分支上的读门.标题级「不是」](./images/redrawn-fig-gated-attn-not-gated-residual.png)
