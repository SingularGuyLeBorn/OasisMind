...
> 前置:[04 MLA — 低秩潜变量与矩阵吸收](../04-MLA-低秩潜变量与矩阵吸收.md)

[04 篇](../04-MLA-低秩潜变量与矩阵吸收.md) 已经定义了 MLA 的数学对象($c^{KV}, c^Q, k^R$ 与解耦 RoPE).本篇**不重推 latent**,只回答工程落地:同一套 checkpoint,Prefill 与 Decode 为什么走不同的计算路径?这两个路径不是两个版本的 MLA,而是同一组权重的两种调度方式;矩阵吸收路径是否等价于完整路径?DeepSeek-V3 参数下何时显式展开、何时用矩阵吸收更省 FLOPs / 激活 / 带宽?
...
很多人第一次复现 MLA 时都会遇到同一句话:「我靠,MLA 怎么根本不省资源?」这通常不是 MLA 不成立,而是把 **常驻 KV cache、临时工作区、权重占用、Prefill 峰值、后端 fallback** 混成了一个数字.
...
**矩阵吸收不是根因,是后端优化**.它把上投影 $W^{UK},W^{UV}$ 的计算重新安排到 Q/O 两侧,减少显式解压、临时完整 K/V 和部分通信/带宽压力.即使不吸收,只要系统确实把 latent 作为持久 KV cache,常驻缓存仍然可以变小;反过来,如果 fallback 先解压并缓存完整 K/V,就会看不到收益.吸收并非无条件更省:在训练或不合适的 fused/eager 路径里,吸收中间量可能变大,省下的是通信/解压路径,不一定是峰值显存.
...
## 1. 同一套 checkpoint,两个阶段的计算路径

![DeepSeek-V3 层:MLA 替换 MHA,与 MoE 正交](./images/redrawn-fig-mla-v3-block-placement.png)
...
- **与 Figure 2(论文 MLA 细图)关系**:本图看「层间堆叠」;04.1 图 2 看「单层 MLA 内部数据流」.

建议:先扫全图找 **Attention / MLA** 标注,再对照 [04 篇](../04-MLA-低秩潜变量与矩阵吸收.md) 公式.

![DeepSeek-V2 MLA 模块(论文 Figure 2)](./images/redrawn-fig-deepseek-v2-mla-appendix-block.png)

> 图 2:论文 Figure 2.斜线块 = inference cache($c^{KV}, k^R$).

**图 2 解析**

与 [04 篇图 2](../04-MLA-低秩潜变量与矩阵吸收.md) 同源(DeepSeek-V2 Figure 2 MLA 放大图).本篇读图时额外强调 **哪些框是 Prefill 完整展开与 Decode 矩阵吸收共用**:

- **$c^{KV}, k^R$ 斜线块**:两个阶段 **cache 内容完全相同**;差别只在算分时是否把 $W^{UK}, W^{UV}$ 提前乘进 Q/O.
- **$W^{UK}, W^{UV}$ 方框**:Prefill 完整展开在 Attention 前显式执行;Decode 矩阵吸收在图 4 中被「拆开」到 Q 后与 O 前.
- **Multi-Head Attention 大框**:Prefill 在 **head 维 $d_{qk}$** 上算;Decode 在 **latent 维 $d_c$** 上算(等价 MQA 内核 + broadcast).

| 阶段 | 工程名 | 核心操作 | KV 上采样 |
|------|--------|----------|-----------|
| **Prefill** | 完整展开 | 并行处理 $L$ 个 token | cache 后 **显式** $W^{UK}, W^{UV}$ |
| **Decode** | 矩阵吸收 | 每步 1 个 token | $W^{UK}$ → Q 侧;$W^{UV}$ → $W^O$ |

![Prefill 完整展开流图](./images/redrawn-fig-mla-nonabsorb-prefill.png)

> 图 3:Attention 在 **完整 head 维** $d_{qk}$ 上算;KV cache 后接上采样.

**图 3 解析**

**Prefill 完整展开** 计算流.按 **数据从左到右,cache 在中间** 读.
...
![Decode 矩阵吸收流图(MQA mode)](./images/redrawn-fig-mla-absorb-decode.png)

> 图 4:Attention 在 **latent 维** $d_c$ 上算(head 维 broadcast);上采样拆到 Q/O 两侧.

**图 4 解析**

**Decode 矩阵吸收(MQA mode)** 计算流(Decode 常用).整体拓扑与图 3 相似,但 **KV 上采样方框被拆开,挪位**.
...
**训练**:始终按式 (1)–(4) 前向(完整展开语义).**推理**:按阶段选完整展开或矩阵吸收实现,权重 tensor **相同**.
...
| 步骤 | 完整展开(Prefill) | 矩阵吸收(Decode, 1 token) |
|------|-------------------|-------------------------|
...
矩阵吸收路径 Attention 在 **$d_c$ 维** 上与 cache 交互,等价 MQA「单份 KV latent」+ head broadcast.
...
#### 4.2.2 Value 侧吸收进 $W^O$(块矩阵形式)

记 $\alpha_{t,j,i}$ 为头 $i$ 的注意力权重.Prefill 完整展开路径下:
...
## 5. 数值走查:Prefill 完整展开与 Decode 矩阵吸收

### 5.1 无 RoPE 的简单例子

$d_c=d_c'=2,\ d_{h,C}=2,\ n_h=1$.$c_1^{KV}=[1,0]^\top,\ c_2^{KV}=[0,1]^\top$;$c_2^Q=[1,1]^\top$;$W^{UQ}=W^{UK}=W^{UV}=I$.

**Prefill 完整展开**:先把每个历史 latent 上投影成头维 K/V:$k_1^C=[1,0]^\top,\ k_2^C=[0,1]^\top$;然后 $S_{2,1}^C=S_{2,2}^C=1$ → $\alpha=[0.5,0.5]$ → $o_2^C=[0.5,0.5]^\top$.

**Decode 矩阵吸收**:直接算 $W_{\mathrm{abs}}=I$,$S_{2,j}^C = c_2^Q (c_j^{KV})^\top$ 与上者相同;$\bar{c}_2^V = 0.5 c_1^{KV}+0.5 c_2^{KV}=[0.5,0.5]^\top = o_2^C$.

矩阵吸收路径 **未 allocate** 完整 $K^C \in \mathbb{R}^{2 \times 2}$ 张量.

### 5.2 含 RoPE 一项

设 $d_h^R=2$,$q_2^R=k_j^R$ 为二维单位向量经 $R_j$ 旋转.即使 $S^C$ 同 §5.1,加上 $S^R$ 后 $\alpha$ 改变,**两条路径仍一致**--只要 (8) 在完整展开与吸收中 **同一式** 计算,且 Value 仍只聚合 $v^C$(RoPE 不进 Value).

Walkthrough:$S_{2,j}=S_{2,j}^C + (R_2 q_0)^\top (R_j k_0)$ → softmax → 式 (11)(13) 分别算 $u_2$,应 bitwise 匹配(忽略浮点误差).

---

## 6. 为什么 Prefill 和 Decode 选不同计算路径

记 $x=\text{seq\_len}$(当前步参与计算的 query 数),$y=\text{cache\_len}$(已有历史).

| 场景 | $x$ | $y$ | 更优 | 原因 |
|------|-----|-----|------|------|
| Prefill | $\gg 1$ | 0 | **完整展开** | 大 $x$ 时 $x(y+x)$ attention GEMM 主导;显式 $K^C,V^C$ 利于 Tensor Core |
| Decode | 1 | $\gg 1$ | **矩阵吸收** | 避免对 $y$ 个历史重复 $c_j^{KV} \mapsto k_j^C$($\propto y n_h d_c d_{h,C}$) |
| Prefix | 中 | 中 | 看交叉点 | 图 7 |
...
**矩阵吸收**:去掉 $F_{kv}$ 中 $n_h(x+y)d_c(d_{h,C}+d_v)$;加 $F_{q,\mathrm{abs}} \sim n_h x d_{h,C} d_c$;$F_{\mathrm{attn}}$ 中 head 维换成 $d_c+d_hR$ 与 $d_c$;$F_o$ 加 $x n_h d_c d_v$.
...
$z = F_{\mathrm{non-abs}} - F_{\mathrm{abs}} < 0$ → 完整展开 FLOPs **更少** → 选 **MHA mode**(与图 5 一致).
...
完整展开 FLOPs **更多** → 选 **MQA mode(矩阵吸收)**(与图 6 一致).

![Prefill diff](./images/redrawn-fig-mla-flops-diff-prefill.png)

> 图 5:$y=0$,$x$ 增大时 $z<0$ 加深 → **完整展开** FLOPs 更少(Prefill 选 MHA mode).

**图 5 解析**

横轴 **seq_len** $x$(Prefill 时当前 batch 长度),纵轴 $z = F_{\mathrm{full}} - F_{\mathrm{abs}}$(完整展开减矩阵吸收的 FLOPs 差).**红色虚线 $z=0$**.

- 固定 **cache_len $y=0$**(纯 Prefill,无历史 cache).
- 曲线全程在 0 线 **下方** → $z<0$ → **完整展开总 FLOPs 更少**.
- $x$ 越大(长 prompt),曲线越负 → 越应选 **MHA mode(图 3)**.

物理含义:长 prompt 并行算时,显式 $K^C,V^C$ 大 GEMM 的 Tensor Core 利用率高;矩阵吸收路径额外的 $W_{\mathrm{abs}}$ 乘在 Prefill 不划算.

![Decode diff](./images/redrawn-fig-mla-flops-diff-decode.png)

> 图 6:$x=1$,$y$ 增大后 $z>0$ → 矩阵吸收更优(Decode 选 MQA mode).

**图 6 解析**

横轴 **cache_len** $y$(已有历史 token 数),纵轴仍为 $z$.固定 **seq_len $x=1$**(Decode 每步只来 1 个新 token).

- 起始 $y=0$ 附近 $z$ 可能 $\le 0$;随 $y$ 增大曲线 **升到 0 线以上** → $z>0$ → **完整展开更费** → 应选 **MQA mode(图 4)**.
- 长对话($y=4096+$)时差距显著:若仍用图 3 对全长 cache 做 KV 上采样,FLOPs $\propto y$ 的浪费极大.

对应工程:**生成阶段默认矩阵吸收路径**.

![Prefix $y=20$](./images/redrawn-fig-mla-flops-diff-prefix-cache.png)
...
![3D 曲面](./images/redrawn-fig-mla-flops-diff-3d.png)
...
- **$z>0$ 区域**(曲面在 0 平面之上):完整展开更费 → 倾向 **矩阵吸收 / MQA mode**.
- **$z<0$ 区域**(曲面之下):完整展开更省 → 倾向 **MHA mode**.
...
### 8.2 KV Cache(与 04 篇 §14一致)

每 token 每层:**576** FP16 元素(512+64)vs MHA 式 32768.

### 8.3 激活峰值(InfraTech 结论)
...
当 $d_c < d_{h,C}$ 且 $y$ 大时,吸收 **激活** 在 Attention 段可更低;但 Q/O 吸收乘引入额外 buffer.Net:Prefill 仍偏完整展开(算力 + 激活);Decode 偏吸收(算力 + 少物化 $K^C$).
...
## 10. Decode 单步走查($x=1,\ y=1000$)

1. 输入 $h_t \in \mathbb{R}^{7168}$(新 token).
2. $c_t^Q, c_t^{KV}$ 各 `[1,512]` / `[1,1536]`;append cache → $y=1001$.
3. **矩阵吸收**:算 $W_{\mathrm{abs},i}$ 得 scores $[128, 1001]$;加 RoPE 项;softmax.
4. $\bar{c}^V = \mathrm{attn} @ c^{KV}_{cache}$ → `[128, 512]`.
5. $u_t = \bar{c}^V @ W^{O'}$ → `[7168]`.

**完整展开**若用于同一步:需先 `up_k/up_v` 得 $K^C,V^C$ shape `[128,1001,128]` → 额外 $\sim 2 \times 128 \times 1001 \times 128 \approx 32M$ 元素物化/读.
...
矩阵吸收模式 Attention 在 **KV 维** 只有 **一份** $c^{KV}$(latent),对 $n_h$ 个 Q 头 broadcast--与 [MQA](../02-MQA-共享KeyValue的极致压缩/02-MQA-共享KeyValue的极致压缩.md) 的「多 Q 单 KV」同构,但:

- MQA 的 KV 维是 $d_h$;MLA 矩阵吸收路径是 $d_c$(通常 $d_c \neq d_h$);
...
| 误解 | 纠正 |
|------|------|
| 矩阵吸收会掉精度 | 精确线性重排;掉点来自 $d_c$ 低秩,完整展开本身 |
...
| 维度 | 完整展开 MHA mode | 矩阵吸收 MQA mode |
|------|-----------------|---------------|
...
**一句话**:[04 篇](../04-MLA-低秩潜变量与矩阵吸收.md) 定义 **cache 是什么**;本篇定义 **同一权重下乘法顺序如何排** 才在 Prefill/Decode 各段最省.式 (9)–(13) 是吸收代数;式 (15) 是选型地图;图 3–8 是工程与实验对照.
