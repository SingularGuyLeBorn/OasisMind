...
MTP（[2.4.6](./2.4.6-多Token预测MTP深度解析.md)）仍在词表上做交叉熵。JEPA（Joint Embedding Predictive Architecture）在 **嵌入空间** 里让一个视图预测另一个视图。LLM-JEPA（Huang, LeCun, Balestriero, [arXiv:2509.14252](https://arxiv.org/abs/2509.14252)）把这件事接到生成式 LLM 上：**保留 $\mathcal{L}_{\mathrm{LLM}}$，再加一项表示预测**。它不是 BERT 式 span masking 的换皮，旧稿那套知乎流程不要当论文。

典型视图：自然语言描述 vs 正则 / SQL / 代码 diff——同一件知识的两种写法，不是「把句子遮掉 30% 再回归隐状态」。

![左：Text 经 Enc 再 Pred；右：Code 经 Enc。余弦对齐表示。叉掉词表 softmax：那不是 MTP](./images/fig-jepa-predict-repr.png)
