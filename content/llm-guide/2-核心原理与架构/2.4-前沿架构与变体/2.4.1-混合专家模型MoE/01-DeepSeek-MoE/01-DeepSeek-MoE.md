...
本篇是 2.4.1 的机制专文，只写 **MoE 层**。注意力侧的 MLA 不在这里推：[2.2.2/04 MLA](../../../2.2-基础注意力机制/2.2.2-多头注意力变体/04-MLA-低秩潜变量与解耦式注意力/04-MLA-低秩潜变量与解耦式注意力.md)。MTP 走 [2.4.6](../../2.4.6-多Token预测MTP深度解析.md)。EP / All-to-All 拓扑走 [08](../../../../6-训练与推理优化/6.1-训练基础设施/6.1.8-MoE系统与并行/08-MoE系统优化综述/08-MoE系统优化综述.md) 与 [6.1.1](../../../../6-训练与推理优化/6.1-训练基础设施/6.1.1-分布式训练/6.1.1-分布式训练.md)。总览里的 Token-Choice、Switch 辅助损失见 [2.4.1](../2.4.1-混合专家模型MoE.md)。
...
> 图 1：DeepSeekMoE 层。上为 $u_t$。左列共享专家全部绿色 ON，每个 token 都算。右列 Router 取 Top-$K_r=2$，只有 $r_2$、$r_4$ 绿色 SELECTED 进求和，灰色 IDLE 不算。输出 $h'_t$。

**图 1 解析**

自上而下读。绿色 = 这个 token 真的跑；灰色 = 没选中、不跑。

- **顶部 $u_t$**：注意力残差之后进入 FFN 插槽的隐藏向量，对应式 (1) 的 $\mathbf{u}_{t}^{l}$。箭头只往下走：进专家顶边，出专家底边，再进 $+$。不是双向，也不是共享专家还要再被路由一次。
- **左列共享专家**：三路都是 ON。没有 Router、没有 Top-K、没有 $g_{i,t}$。实现上就是 $N_s$ 次较窄的 SwiGLU-FFN，输出相加，对应式 (9) 第一项。
- **右列 Router / Top-$K_r$**：先打分再留两个最大的。图里这一次 $r_2=0.33$、$r_4=0.29$ 入选；$r_1$、$r_3$、$N_r$ 灰色 IDLE，$g=0$，那几路 FFN 不跑。条上的数字是门控，不是注意力权重。
- **选中的路由专家**：只画出 expert 2 和 expert 4，各自乘 $g$，对应式 (9) 第二项。
- **底部 $+$ → $h'_t$**：共享三路（权重恒 1）加上两路加权路由。残差 $u_t$ 写在式 (9) 末项，图的求和式没画 skip，避免和共享支路挤在一起。
...
> 图 2：左栏稠密块的黄色 Dense FFN 被右栏绿色 DeepSeekMoE 层替换；蓝色 Self-Attention / MLA 保持原位。算法视角：残差后的 $u_t$ 派发到 $N_s$ 个共享专家与 $K_r$ 个路由专家再求和。

**图 2 解析**

- **左栏自下而上**：$\mathbf{h}^{l-1}$ → 注意力 → 残差得 $\mathbf{u}^{l}$ → 稠密 FFN → 残差得 $\mathbf{h}^{l}$。红虚线框住的是**唯一被换掉的插槽**。
- **右栏**：注意力子层仍是 MLA，KV 压缩公式在注意力那一章。绿盒吃的是注意力残差后的 $u_t$，吐出的 $h'_t$ 已含式 (12) 里的 $u_t$ 残差。
...
V2 训练里共享专家计算与专家并行的 All-to-All 重叠，那是把 always-on 支路变成通信掩盖，**不改变**式 (12)。真正的 EP 映射、节点内 NVLink / 跨节点 IB，见 [08](../../../../6-训练与推理优化/6.1-训练基础设施/6.1.8-MoE系统与并行/08-MoE系统优化综述/08-MoE系统优化综述.md) 与 [6.1.1](../../../../6-训练与推理优化/6.1-训练基础设施/6.1.1-分布式训练/6.1.1-分布式训练.md)，本篇不把通信拓扑当主叙事。
...
## 7. V2：每个 token 最多进 $M$ 台设备

细粒度之后 $K_r$ 变大，一个 token 的目标专家容易散落在很多设备上。真正贵的是「这个 token 要跟几台机器说话」，不是「选了几个专家」。V2 在 Top-$K_r$ 之外加一条硬限制：每个 token 的目标专家最多落在 $M$ 台设备上。步骤：

1. 按设备聚合其上专家的亲和度，选出总分最高的 $M$ 台；
2. **只在这 $M$ 台内部的专家里**再做 Top-$K_r$。

论文写 $M\geqslant 3$ 时，与无约束 Top-$K$ 大致对齐。V2 取 $M=3$。改的是路由器还允许选哪些专家，不是 InfiniBand 上 All-to-All 怎么发包。V3 改成**节点**受限：每 token 最多 $M=4$ 个节点，节点按其上最高 $K_r/M$ 个亲和度之和来挑。机器之间怎么走 IB、再经 NVLink 转发，见 [08](../../../../6-训练与推理优化/6.1-训练基础设施/6.1.8-MoE系统与并行/08-MoE系统优化综述/08-MoE系统优化综述.md)。
...
> 图 3：左栏 V1/V2：全局 Softmax → Top-$K_r$ → 分数直接当 $g$（和不为 $1$）。右栏 V3：逐专家 Sigmoid → $b_i$ 仅用于排序 → Top-$K_r$ → 用原始 $s$ 在选中集合上归一化。中间标注：不是 Qwen 的「先 Top-K 再 Softmax」。
...
MLA 公式链回 [04](../../../2.2-基础注意力机制/2.2.2-多头注意力变体/04-MLA-低秩潜变量与解耦式注意力/04-MLA-低秩潜变量与解耦式注意力.md)；KV 字节参照系是 [01 MHA](../../../2.2-基础注意力机制/2.2.2-多头注意力变体/01-MHA-多头注意力的标准形式/01-MHA-多头注意力的标准形式.md)。MTP 见 [2.4.6](../../2.4.6-多Token预测MTP深度解析.md)。容量、z-loss、drop 写在 [2.4.1 第 4–5 节](../2.4.1-混合专家模型MoE.md)。
