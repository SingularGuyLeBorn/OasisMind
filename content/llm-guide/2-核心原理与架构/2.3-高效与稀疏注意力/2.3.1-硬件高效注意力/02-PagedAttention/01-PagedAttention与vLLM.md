...
> **2026-08 修订（不删上文）。** §1.4 / §2.2 / §9.1 把故事写成「GPU 利用率 30%→80%+」「100 张 A100 变 40 张、一年省 100 万美元」——这些 **不在** [2309.06180](https://ar5iv.labs.arxiv.org/html/2309.06180)。论文不报 nsys SM% 对照，也不做云账单。对照系统是 FasterTransformer 与三种 Orca 内存策略（Oracle / Pow2 / Max）。摘要吞吐 **2–4×（同等延迟）**；ShareGPT 请求率相对 Orca (Oracle) **1.7–2.7×**、Orca (Max) **2.7–8×**、FasterTransformer 最高约 **22×**。KV 有效占用现有系统 **20.4%–38.2%**，vLLM near-zero（浪费 ≤ 最后一块）。共享用块数比：并行采样 / beam 的百分数见 [2.3.4 修订](../../2.3.4-高效注意力全景综述/2.3.4-高效注意力全景综述.md)，不要用「典型 50–90%」。A100 40GB 13B 布局约 65% 权重 + 30% KV。注意力 kernel **更慢 20–26%**，所以也不是「无代价」。70B FP16「140 GB 单卡」那段是另一套算账，不要和这篇 13B/A100-40GB 评测混成一张表。
