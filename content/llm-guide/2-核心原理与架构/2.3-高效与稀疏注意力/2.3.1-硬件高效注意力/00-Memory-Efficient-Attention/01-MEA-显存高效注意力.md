...
4. Llama-1 训练段：xFormers 因果 MHA「inspired by Rabe and Staats (2021) and uses the backward from Dao et al. (2022)」。本库旧稿曾把这写成「类似 FlashAttention 的变体」——那是工程拼接（xFormers 前向灵感来自本篇 JAX/TPU 算法，反向用 Dao et al. 2022），不是把 2112.05682 搬进 PyTorch。API 名 `memory_efficient_attention` 也不能当论文身份；模型身份与已核证口径见 [LLaMA](../../../../05-模型家族与选型/5.3-模型家族/llama/llama/llama.md)，对照见 [01-Attention实现方式全景对比](../04-Attention实现方式对比/01-Attention实现方式全景对比.md)。
