---
title: "8.7 · Omni 与全双工：级联、端到端、时间窗"
date: 2026-08-30
as_of: 2026-08-30
...
<!-- GPT-Image-2 Prompt: Technical educational diagram of omni multimodal interaction on a white academic background, no watermark, no logo, no copyright text, no stock-photo banner, no website URL. Three horizontal columns, research-paper style, blue and orange arrows, readable labels, no decorative art. Column 1 Cascade pipeline: audio waveform to ASR box to text tokens to LLM box to text reply to TTS box to waveform, stacked delays labeled serial. Column 2 End-to-end speech LM: waveform to speech tokenizer discrete tokens into one Transformer then speech decoder back to waveform, interleaved with text tokens. Column 3 Full-duplex Omni-Flow: shared time axis with three parallel streams labeled env-visual, env-audio, out-stream, time windows of duration t, listen-or-speak control token then content. Minimal academic palette, precise arrows, clean typography. -->
...
| CLIP / VLM 投影 | 积木 | [8.8 CLIP](../8.8-CLIP与视觉编码器/01-CLIP与视觉编码器.md)、[LLaVA](../8.2-视觉语言模型/8.2.1-LLaVA架构深度解析.md) | omni 仍可能用 ViT+投影，不替代对齐课 |
