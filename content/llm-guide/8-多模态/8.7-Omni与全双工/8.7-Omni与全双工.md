...
published: true
excerpt: 第 8 章 omni 地图。把「能看能听能说」拆成三个不同瓶颈：级联、端到端 SpeechLM、全双工时间窗。版本身份与发布配方在第 05 章已有正本，这里只画依赖。
---
...
>
> 模型侧精读（只链不抄）：[GLM-4-Voice](../../05-模型家族与选型/5.3-模型家族/glm/glm-4-voice/glm-4-voice.md) · [MiniCPM-o 4.5](../../05-模型家族与选型/5.3-模型家族/minicpm/minicpm-o-4-5/minicpm-o-4-5.md) · [GPT-4o 公开材料](../../05-模型家族与选型/5.3-模型家族/openai/gpt-4-series/gpt-4-series.md) · [Project Astra](../../05-模型家族与选型/5.3-模型家族/gemini/project-astra/project-astra.md)
...
即每帧大约 $2^{14}$ 的码本容量。帧率再降到 6.25 Hz（表里 100 bps）时，LibriSpeech-clean 上的 ASR WER 从 12.5 Hz 的 2.10 跳到 14.41——语义已经不够用。12.5 Hz 相对 50 Hz（600 bps）在 MOSNet 上几乎打平（3.39 vs 3.38），所以他们拿码率换序列长度。码本用 EMA（衰减 0.99）更新，低频向量会重置，这是防 codebook collapse 的标准招，细节见 [GLM-4-Voice 正本](../../05-模型家族与选型/5.3-模型家族/glm/glm-4-voice/glm-4-voice.md)。
...
第 05 章已有演示场景叙事：[Astra 正本](../../05-模型家族与选型/5.3-模型家族/gemini/project-astra/project-astra.md)。本体系章只收三句边界：

1. Astra 证明 omni 的产品形态是 **持续感知 + 记忆 + 工具**，不只是语音码本。
2. 记忆、工具、沙箱的本体在第 13 章，不在第 8 章再写一遍 Agent。
3. 没有技术报告就不要在模型家族页假装有 D2 全文翻译。闭源演示当系统叙事，数字以官方页能核对的为准。

GPT-4o 同理：公开材料够写「单网、多模态 I/O、232/320 ms」，不够写码率和全双工窗长。第 05 章公开材料正本已经承担叙事；本篇不把推测性 tokenizer 画进公式。

## 7. 地图：trick 住哪，第 8 章不再复制

| 机制 | 解决哪面（0.4） | 本体文 | 本篇只记住 |
|------|-----------------|--------|------------|
| 监督语义 tokenizer 12.5 Hz / 175 bps | 积木 + 数据 | [GLM-4-Voice](../../05-模型家族与选型/5.3-模型家族/glm/glm-4-voice/glm-4-voice.md) | 单码本、中间 VQ、流式因果 |
| 交错语音-文本继续预训练 | 数据 | 同上 | 从文本 LLM 迁知识，不是堆真实小时的唯一解 |
| streaming thoughts | 训推 | 同上 | 文本/语音交替，降首包 |
| Omni-Flow + LS + TAIL | 架构 + 训推 | [MiniCPM-o 4.5](../../05-模型家族与选型/5.3-模型家族/minicpm/minicpm-o-4-5/minicpm-o-4-5.md) | $t=1.0\,\mathrm{s}$；控制与内容拆开 |
| 早期 omni 实时栈 | 架构 | [MiniCPM-o 2.6](../../05-模型家族与选型/5.3-模型家族/minicpm/minicpm-o-2-6/minicpm-o-2-6.md) | 4.5 之前的端到端实时，尚未 Omni-Flow |
| 单网 omni（公开） | 架构（闭源） | [GPT-4o](../../05-模型家族与选型/5.3-模型家族/openai/gpt-4-series/gpt-4-series.md) | 232 / 320 ms；无公开码率 |
| 持续感知助手 | 系统 / Agent | [Astra](../../05-模型家族与选型/5.3-模型家族/gemini/project-astra/project-astra.md) | 原型；能力进 Gemini Live |
| CLIP / VLM 投影 | 积木 | [8.8 CLIP](../8.8-CLIP与视觉编码器/8.8-CLIP与视觉编码器.md)、[LLaVA](../8.2-视觉语言模型/8.2.1-LLaVA架构深度解析.md) | omni 仍可能用 ViT+投影，不替代对齐课 |

第 05 章负责版本身份与来源，第 8.7 负责可跨家族复用的机制和失效条件。禁止在每个新 omni 模型目录里再贴一遍 Table 1。
