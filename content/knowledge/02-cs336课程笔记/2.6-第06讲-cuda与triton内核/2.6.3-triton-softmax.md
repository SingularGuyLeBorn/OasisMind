---
title: "triton softmax"
category: "CS336 课程笔记"
tags: [CS336, 语言模型]
published: true
excerpt: "本笔记将解析如何使用Triton实现一个高性能的Softmax核函数. 与逐元素的GeLU不同, Softmax是一个行级 row-wise 操作, 并且包含归约 reduction 步骤 求最大值, 求和. 这使得它成为展示Triton处理更复杂并行模式的绝佳案例."
---
# triton softmax
...
这个为Softmax量身定制的"一行一块"策略, 是在掌握了Triton基础之上的进阶应用. 关于Triton的块级编程范式, 向量化加载 (`tl.load`) 和掩码 (`mask`) 等基础概念, 读者可以先参考 [Triton GeLU笔记](./2.6.2-triton-gelu.md) 以建立更扎实的基础.
