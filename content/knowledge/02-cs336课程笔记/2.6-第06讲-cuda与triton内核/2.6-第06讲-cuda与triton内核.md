...
tags: [CS336, 语言模型]
published: true
excerpt: "大家好. 在本次课程中, 我们将深入探讨如何为GPU编写高性能代码. 第二次作业的部分内容就会要求大家进行大量的性能分析profiling, 甚至为Flash Attention V2编写自己的Triton核函数kernel, 致力于实现极致的性能. 因此, 本次课程的目标就是为构建语言模型中的标准"
---
# 第六讲:手写高性能算子
...
| 层级 | 典型作用 | 相对特征 |
|---|---|---|
| 寄存器 | 单线程的标量与中间值 | 最低延迟、容量最小 |
| 共享内存 / L1 | 线程块内复用数据 | 片上、需显式组织访问 |
| L2 | 多 SM 共享缓存 | 容量与延迟居中 |
| HBM / DRAM | 模型参数、激活与大数组 | 容量最大、数据移动代价最高 |
...
- [剖析CPU与GPU的异步执行模型](./2.6.4-cpu-gpu异步.md)
...
| 实现方式 | 全局内存往返 | 中间结果 |
|---|---:|---|
| 多个独立 kernel | 每个 kernel 都可能读写 HBM | 多次物化 |
| 融合 kernel | 主要在入口读取、出口写回 | 尽量留在寄存器或片上存储 |
...
- [代码解析: 手写CUDA C++ GeLU核函数](./2.6.1-cuda-gelu.md)
...
- [代码解析: 使用Triton编写GeLU核函数](./2.6.2-triton-gelu.md)
...
- [代码解析: 使用Triton实现高性能Softmax](./2.6.3-triton-softmax.md)
