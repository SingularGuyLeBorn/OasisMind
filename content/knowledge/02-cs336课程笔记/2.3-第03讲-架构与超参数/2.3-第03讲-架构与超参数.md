---
title: "CS336  关于语言模型架构与训练的一切 - 讲座3笔记"
category: "CS336 课程笔记"
tags: [CS336, 语言模型]
published: true
excerpt: "本讲座将深入探讨Transformer架构的演进细节和训练中的超参数选择,这些细节往往在其他课程中被略过. 我们将从一个更偏向数据驱动的视角,通过分析过去几年中发布的大量大型语言模型LLM,来理解哪些架构选择是至关重要的,哪些已经成为共识."
---
...
![oOtqV3jTy0DikZ7](.files/oOtqV3jTy0DikZ7.png)

### 现代 Transformer 变体

然而,你在课程作业中实现的并非原始版本,而是一个更现代的变体. 主要区别在于:

- **规范化层前置 (Pre-Norm)** : [Normalization Strategies in Transformers](./2.3.3-归一化策略.md) 是现代架构的基石之一. 规范化层(LayerNorm)被移到了注意力层和FFN层**之前**.
- **旋转位置编码 (RoPE)** : 我们使用 [Rotary Position Embeddings (RoPE)](./2.3.4-旋转位置编码.md),这是一种更先进的位置编码方法.
- **门控激活单元 (GLU)** : 前馈网络层(FFN)使用 [Gated Activations (GLU)](./2.3.1-门控激活.md),特别是SwiGLU,而不是标准的ReLU.
- **无偏置项**: 线性层和规范化层中通常省略了偏置(bias)项.

![ZDU870abJbALu0M](.files/ZDU870abJbALu0M.png)
...
![gJZJgBZjw5NUxBK](.files/gJZJgBZjw5NUxBK.png)
...
![javRkmQ7APH89zq](.files/javRkmQ7APH89zq.png)
...
![0dJIRaHjNaluDul](.files/0dJIRaHjNaluDul.png)
...
![p0U9TRASIGvgDkx](.files/p0U9TRASIGvgDkx.png)
...
![evhAeHIAMXIziUo](.files/evhAeHIAMXIziUo.png)
...
![XZbbYQRhliXhWmi](.files/XZbbYQRhliXhWmi.png)

这两个选择的背后,是业界对**训练稳定性**和系统效率(尤其是内存带宽)的深刻理解.

> **[深度解析]** 关于 `Pre-Norm`为何优于 `Post-Norm`,`RMSNorm`相比 `LayerNorm`的效率优势,以及最新的“双重规范化”趋势,请参阅 [Normalization Strategies in Transformers](./2.3.3-归一化策略.md).

> **[深度解析]** 为什么将层归一化(Layer Normalization, LN)放在残差连接内部(即 Pre-Norm)通常是更好的选择 [为什么这样做是对的？](./2.3.5-归一化层位置.md)
...
![NVfFLLRWXqdtNMJ](.files/NVfFLLRWXqdtNMJ.png)

![UQnJoPZ7x08rmkK](.files/UQnJoPZ7x08rmkK.png)
...
> **[深度解析]** 门控机制是什么？为什么它比简单的非线性函数更有效？请参阅 [Gated Activations (GLU)](./2.3.1-门控激活.md).
...
> **[深度解析]** RoPE如何通过旋转实现相对位置编码？其背后的数学原理和实现细节是怎样的？请参阅 [Rotary Position Embeddings (RoPE)](./2.3.4-旋转位置编码.md).
...
![7JJtbYC33Pu5hzA](.files/7JJtbYC33Pu5hzA.png)

````
Jared Kaplan的论文中有一个关键图表
````

![D6iq8hhmh1A3wOn](.files/D6iq8hhmh1A3wOn.png)
...
![AHoG1qLikrFabdf](.files/AHoG1qLikrFabdf.png)
...
![vkOjeAuT3EwMlko](.files/vkOjeAuT3EwMlko.png)

- 这个选择也受到系统并行化策略的影响:极深的模型有利于流水线并行,而极宽的模型有利于张量并行.

![wPEUGmk9OJL0dog](.files/wPEUGmk9OJL0dog.png)

![rQPB9pSEGPFQPcI](.files/rQPB9pSEGPFQPcI.png)
...
> **[深度解析]** 为何推理时的计算模式与训练时完全不同？KV缓存瓶颈是如何产生的？MQA/GQA又是如何从根本上解决这个问题的？请参阅 [Grouped-Query & Multi-Query Attention (GQA/MQA)](./2.3.2-gqa-mqa与mla.md).   实际上课上并没有MLA,但是作为2025年下半年的这类文章MLA还是必不可少的,甚至NSA MoBA也不能少
...
![ZRWaKIzNgfGsiws](.files/ZRWaKIzNgfGsiws.png)
...
![2kNm75WinJhpn5F](.files/2kNm75WinJhpn5F.png)
