...
tags: [CS336, 语言模型]
published: true
excerpt: "在深入探讨各种注意力机制之前,我们首先统一定义本文中将使用的变量. 明确的变量命名有助于我们精确地理解每个计算步骤的含义."
---
# **变量定义**
...
![MPVUL6QYTu6L9G9](.files/MPVUL6QYTu6L9G9.png)
...
| 注意力形式 | 每层需要缓存的 K/V 头数 | KV Cache 相对量级 |
|---|---:|---:|
| MHA | $h$ | $O(2hLd_h)$ |
| GQA | $g$，且 $1<g<h$ | $O(2gLd_h)$ |
| MQA | $1$ | $O(2Ld_h)$ |

其中 $L$ 是已缓存序列长度，$h$ 是查询头数，$g$ 是 K/V 组数。缓存随上下文长度线性增长；达到服务系统上限时通常会触发拒绝、截断或分页换出，并不意味着缓存会无提示地覆盖旧内容。
...
![q2RkH83IUSPRQGR](.files/q2RkH83IUSPRQGR.jpg)



![Cby5OElrnHBN5RX](.files/Cby5OElrnHBN5RX.jpg)
...
![MF96vdyXq1Q101e](.files/MF96vdyXq1Q101e.jpg)
...
![13ArblD3v4ZkTg7](.files/13ArblD3v4ZkTg7.jpg)
...
![DUnd88nsYwXY6jx](.files/DUnd88nsYwXY6jx.png)

![aI6o1VQ81EZaCMx](.files/aI6o1VQ81EZaCMx.png)
