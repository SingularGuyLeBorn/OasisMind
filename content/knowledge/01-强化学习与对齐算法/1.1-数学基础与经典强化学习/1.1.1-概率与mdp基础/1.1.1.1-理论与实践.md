---
title: "理论与实践的桥梁: 从测度到代码 (Theory to Practice)"
category: "强化学习与对齐算法"
tags: [强化学习, 对齐]
published: true
excerpt: "你在 02Implementation.py 中看到的只有 np.random.normal 和 mean，并没有显式的 $\\sigma$-代数或勒贝格积分. 这是为什么？"
---
# 理论与实践的桥梁: 从测度到代码 (Theory to Practice)

## 1. 为什么代码里见不到测度论？

你在 `implementation.py` 中看到的只有 `np.random.normal` 和 `mean()`，并没有显式的 $\sigma$-代数或勒贝格积分. 这是为什么？
