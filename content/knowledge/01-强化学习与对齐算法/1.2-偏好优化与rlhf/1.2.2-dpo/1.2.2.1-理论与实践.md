---
title: "DPO: 从公式到代码的实现指南"
category: "强化学习与对齐算法"
tags: [强化学习, 对齐]
published: true
excerpt: "本文档解释如何将DPO的数学公式转化为代码，以及关键的工程实现细节."
---
# DPO: 从公式到代码的实现指南
...
```python
def compute_log_probs(logits, labels, response_mask):
    ...
    # 求和得到序列log概率
    return (per_token_logps * response_mask).sum(dim=-1)
```

**关键实现细节**:

1. **对齐问题**: 语言模型的logits[t]预测的是token[t+1]
2. **掩码处理**: `response_mask` 只覆盖回答 token；prompt 和 padding 都必须为 0，普通 `attention_mask` 不能直接替代
...
## IPO：在未乘 beta 的偏好边际 h 上回归到 1/(2 beta)
h = logratios_diff
loss = (h - 1 / (2 * beta)) ** 2
