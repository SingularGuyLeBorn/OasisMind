"""
# 实现状态：最小可运行；含损失函数级断言，未运行完整论文训练复现。
...
3. 在已有参考策略与偏好数据的前提下，绕过显式奖励模型与在线 PPO 阶段
...
class DPOConfig:
    ...
    Attributes:
        ...
        reference_free: 是否使用本文提供的 SimPO 风格长度归一目标
        simpo_gamma: SimPO 目标奖励边际
    """
    beta: float = 0.1
...
def compute_dpo_loss(
    ...
    # 步骤2: 先保留未乘 beta 的偏好边际 h；DPO logits = beta * h。
    h = pi_logratios_chosen - pi_logratios_rejected
    logits = beta * h
    ...
    if loss_type == "sigmoid":
        ...
        # IPO 在未乘 beta 的 h 上回归到 1/(2 beta)。
        if beta <= 0:
            raise ValueError("IPO 要求 beta > 0")
...
def compute_dpo_loss_reference_free(
    ...
    """
    SimPO 风格的无参考、长度归一偏好目标。

    两个输入必须已经是回答区间的平均 token log-prob，而不是序列求和；
    否则长短回答会获得不同的隐式尺度，便不再是这里声明的 SimPO 形式。
    ...
    with torch.no_grad():
        reward_accuracies = (policy_chosen_avg_logps > policy_rejected_avg_logps).float()
...
def compute_log_probs(
    ...
    Args:
        ...
        response_mask: 只标记回答 token；prompt 与 padding 必须为 0
    ...
    logits = logits[:, :-1, :]
    response_mask = response_mask[:, 1:].clone()
    token_counts = response_mask.sum(dim=-1)
    if torch.any(token_counts <= 0):
        raise ValueError("each preference response needs at least one scored token")
...
class DPOTrainer:
    ...
    def compute_reference_log_probs(
        ...
        if self.ref_model is None:
            raise ValueError("标准 DPO/IPO 需要参考模型；仅 reference_free=True 时可省略")
        with torch.no_grad():
            ref_logits = self.ref_model(
                input_ids=input_ids,
                attention_mask=attention_mask
            ).logits
            ref_log_probs = compute_log_probs(ref_logits, labels, response_mask)
    ...
    def train_step(
        ...
        Args:
            chosen_*: 人类偏好的 response；attention mask 用于前向，response mask 用于损失
            rejected_*: 人类不偏好的 response；prompt token 不得进入序列 log-prob
        ...
        # 2. 根据目标选择序列求和（DPO/IPO）或回答区间平均（SimPO）。
        if self.config.reference_free:
            policy_chosen_logps = compute_log_probs(
                policy_chosen_logits, chosen_labels, chosen_response_mask,
                average_log_prob=True,
            )
            policy_rejected_logps = compute_log_probs(
                policy_rejected_logits, rejected_labels, rejected_response_mask,
                average_log_prob=True,
            )
            loss, metrics = compute_dpo_loss_reference_free(
                policy_chosen_logps, policy_rejected_logps,
                self.config.beta, self.config.simpo_gamma
            )
        else:
            policy_chosen_logps = compute_log_probs(
                policy_chosen_logits, chosen_labels, chosen_response_mask
            ...
            ref_chosen_logps = self.compute_reference_log_probs(
                chosen_input_ids, chosen_attention_mask, chosen_labels, chosen_response_mask
            )
            ref_rejected_logps = self.compute_reference_log_probs(
                rejected_input_ids, rejected_attention_mask, rejected_labels, rejected_response_mask
            )
...
if __name__ == "__main__":
    ...
    # IPO 常数项断言：beta=0.1 时，未缩放边际 h 的最优点是 5。
    ipo_loss, _ = compute_dpo_loss(
        torch.tensor([5.0]), torch.tensor([0.0]),
        torch.tensor([0.0]), torch.tensor([0.0]),
        beta=0.1, loss_type="ipo"
    )
    assert torch.allclose(ipo_loss, torch.tensor(0.0))
    ...
    response_mask = torch.ones(batch_size, seq_len)  # 此演示没有单独的 prompt 区间
    ...
    with torch.no_grad():
        ...
        policy_chosen_logps = compute_log_probs(
            policy_chosen.logits, chosen_ids, response_mask
        )
        policy_rejected_logps = compute_log_probs(
            policy_rejected.logits, rejected_ids, response_mask
        )
        ref_chosen_logps = compute_log_probs(
            ref_chosen.logits, chosen_ids, response_mask
        )
        ref_rejected_logps = compute_log_probs(
            ref_rejected.logits, rejected_ids, response_mask
        )
