---
title: "直接偏好优化 (Direct Preference Optimization, DPO)"
category: "强化学习与对齐算法"
tags: [强化学习, 对齐]
published: true
excerpt: "论文信息: - 标题: Direct Preference Optimization: Your Language Model is Secretly a Reward Model - 作者: Rafael Rafailov, Archit Sharma, Eric Mitchell, Stefan"
---
...
**前置知识**: PPO(1.1.7 节)、RLHF基本流程

---

## 0. 本章目标

DPO 是 LLM 偏好优化的重要方法。论文在 KL 正则化奖励最大化和 Bradley–Terry 偏好建模的假设下，将最优策略改写为可直接优化的偏好损失，不需要在训练循环中单独拟合显式奖励模型。

本章将:

1. 回顾 PPO 式 RLHF 的数据流与工程成本
2. 在上述假设下推导 DPO 的隐式奖励表达
3. 说明 DPO 目标与 KL 正则化 RL 问题之间的对应边界
4. 比较 DPO 相对 PPO 式流水线的简化点和失效模式
5. 定位 IPO、SimPO 与 ORPO 等相关方法
