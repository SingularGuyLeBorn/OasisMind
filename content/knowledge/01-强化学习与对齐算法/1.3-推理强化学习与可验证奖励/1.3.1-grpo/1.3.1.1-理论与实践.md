---
title: "GRPO：逐 token surrogate 与损失归约"
category: "强化学习与对齐算法"
tags: [强化学习, 对齐]
published: true
excerpt: "用张量形状说明 GRPO 的逐 token ratio、组优势、mask，以及原始 GRPO、Dr. GRPO 与 DAPO 的归约差别。"
---
# GRPO：逐 token surrogate 与损失归约

> 实现状态：教学骨架；未作为完整训练系统或论文结果复现验证。

本文档固定张量语义：`new_log_probs`、`old_log_probs`、`response_mask` 的形状均为 `[B,T]`，`advantages` 为 `[B]`。只有 response 的有效 token 进入 mask。
...
**公式**:
$$\rho_{i,t}(\theta)=\exp\left(\log\pi_\theta(y_{i,t}|x,y_{i,<t})-\log\pi_{old}(y_{i,t}|x,y_{i,<t})\right)$$

**代码实现**:

```python
## ratio 保留 [B,T]，旧策略停止梯度
...
surrogate = torch.minimum(ratio * adv_token, clipped_ratio * adv_token)
surrogate = surrogate * response_mask

## 原始 GRPO：先按每条回答的有效 token 平均，再按回答平均
lengths = response_mask.sum(-1).clamp_min(1)
loss = -(surrogate.sum(-1) / lengths).mean()
```

以下写法不是 GRPO：`exp(((new-old)*mask).sum(-1))`。它把整条回答压成一个 sequence ratio，改变了目标身份。
...
### 3.3 原始 GRPO、Dr. GRPO 与 DAPO 的归约

```python
## 原始 GRPO：每条回答等权
grpo = (surrogate.sum(-1) / response_mask.sum(-1).clamp_min(1)).mean()

## Dr. GRPO：固定生成预算，并且优势不除组内标准差
dr_grpo = (surrogate.sum(-1) / generation_budget).mean()

## DAPO：批内所有有效 response token 共用一个分母
dapo = surrogate.sum() / response_mask.sum().clamp_min(1)
```

三者都使用逐 token importance ratio。区别发生在优势标准化、裁剪边界和 loss reduction，而不是 GRPO 把 token ratio 相乘。
